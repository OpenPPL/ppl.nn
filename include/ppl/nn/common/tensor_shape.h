...
public:
    ...
    uint32_t GetDimCount() const {
        if (data_format_ == ppl::common::DATAFORMAT_SCALAR) {
            return 1;
        }
        return dims_.size();
    }
    ...
    void SetDataFormat(ppl::common::dataformat_t data_format) {
        ...
        if (data_format != data_format_) {
            ...
            if (data_format == ppl::common::DATAFORMAT_SCALAR) {
                dims_.clear();
                padding0_.clear();
                padding1_.clear();
            } else {
    ...
    uint64_t GetElementsIncludingPadding() const {
        if (dims_.empty()) {
            return (data_format_ == ppl::common::DATAFORMAT_SCALAR) ? 1 : 0;
        }
        uint64_t accu = 1;
        for (uint32_t i = 0; i < dims_.size(); ++i) {
    ...
    bool IsScalar() const {
        return (data_format_ == ppl::common::DATAFORMAT_SCALAR);
    }
