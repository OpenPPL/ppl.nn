...
RetCode ReshapeKeyValueCache(InputOutputInfo* info, const ir::Attr* arg, const int64_t* start_pos_data) {
    ...
    out_dims[1] = current_key_shape.GetDim(1) + start_pos_data[0];
...
RetCode ReshapeKeyValueCache(InputOutputInfo* info, const ir::Attr* arg) {
    auto start_pos_data = info->GetInput<TensorImpl>(2)->GetBufferPtr<int64_t>();
    if (!start_pos_data) {
        LOG(DEBUG) << "ERROR: start_pos's data is empty.";
        return RC_NOT_FOUND;
    }
