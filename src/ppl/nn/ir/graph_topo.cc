...
bool GraphTopo::RenameNode(Node* node, const string& new_name) {
    ...
    node->shared_name_str_ = &ret_pair.first->first;
    return true;
}
...
static uint32_t FindEdgeIdIdx(const string& name, const vector<edgeid_t>& edge_ids, const GraphTopo* topo) {
    for (uint32_t i = 0; i < edge_ids.size(); ++i) {
        auto eid = edge_ids[i];
        auto edge = topo->GetEdge(eid);
        if (edge && edge->GetName() == name) {
...
uint32_t GraphTopo::GetInputIdx(const string& name) const {
