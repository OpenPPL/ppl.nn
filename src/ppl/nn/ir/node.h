...
public:
    Node(nodeid_t id, const std::string* shared_name_str) : id_(id), shared_name_str_(shared_name_str) {}
...
private:
    ...
    const nodeid_t id_;
    const std::string* shared_name_str_; // pointer to GraphTopo::name2nid_[name]::first
    Type type_;
