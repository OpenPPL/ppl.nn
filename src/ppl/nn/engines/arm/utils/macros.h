...
#ifndef _ST_HPC_PPL_NN_ENGINES_ARM_UTILS_MACROS_H_
#define _ST_HPC_PPL_NN_ENGINES_ARM_UTILS_MACROS_H_

#include "ppl/nn/common/logger.h"
#include "ppl/common/stripfilename.h"

#ifndef PPLNN_ARM_DEBUG_TRACE
...
#define PPLNN_ARM_DEBUG_TRACE(fmt, ...)
#endif // DEBUG
#endif // Not define PPLNN_ARM_DEBUG_TRACE

#define PPL_ARM_TENSOR_PRINT_DEBUG_MSG(X)                                                                        \
    do {                                                                                                         \
        if (X->GetShape()->IsScalar()) {                                                                          \
            PPLNN_ARM_DEBUG_TRACE("Scalar name: %s\n", X->GetName());                                            \
            PPLNN_ARM_DEBUG_TRACE("\tdata: %p type: %u\n", X->GetBufferPtr(), X->GetShape()->GetDataType());      \
        } else {                                                                                                 \
            PPLNN_ARM_DEBUG_TRACE("Tensor name: %s\n", X->GetName());                                            \
            PPLNN_ARM_DEBUG_TRACE("\tdata: %p\n", X->GetBufferPtr());                                            \
            PPLNN_ARM_DEBUG_TRACE("DimCount: %u\n", X->GetShape()->GetDimCount());                                \
            for (uint32_t i = 0; i < X->GetShape()->GetDimCount(); ++i) {                                         \
                PPLNN_ARM_DEBUG_TRACE("\tdim[%u]: %ld\tpads: [%hu, %hu]\n", i, X->GetShape()->GetDim(i),          \
                                      X->GetShape()->GetPadding0(i), X->GetShape()->GetPadding1(i));               \
            }                                                                                                    \
        }                                                                                                        \
        PPLNN_ARM_DEBUG_TRACE("DataType: %s\n", ppl::common::GetDataTypeStr(X->GetShape()->GetDataType()));       \
        PPLNN_ARM_DEBUG_TRACE("DataFormat: %s\n", ppl::common::GetDataFormatStr(X->GetShape()->GetDataFormat())); \
    } while (0)

#define PPLNN_ARM_REQUIRED_INPUT(X, IDX) \
...
#define PPLNN_ARM_OPTIONAL_INPUT(X, IDX) \
    auto X = ctx->GetInputCount() > IDX ? ctx->GetInput<TensorImpl>(IDX) : nullptr

#define PPLNN_ARM_REQUIRED_OUTPUT(X, IDX) \
...
#define PPLNN_ARM_OPTIONAL_OUTPUT(X, IDX) \
    auto X = ctx->GetOutputCount() > IDX ? ctx->GetOutput<TensorImpl>(IDX) : nullptr
