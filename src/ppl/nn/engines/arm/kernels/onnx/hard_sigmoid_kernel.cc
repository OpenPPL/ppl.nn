...
#include "ppl/kernel/arm_server/hard_sigmoid/neon/hard_sigmoid.h"
...
ppl::common::RetCode HardSigmoidKernel::DoExecute(KernelExecContext* ctx) {
    ...
    return ppl::kernel::arm_server::neon::hard_sigmoid(input->GetShape(), input->GetBufferPtr<void>(), param_->alpha,
                                                       param_->beta, output->GetBufferPtr<void>());
}
