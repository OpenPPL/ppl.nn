...
#ifndef __ST_PPL_KERNEL_ARM_SERVER_PAD_NEON_PAD_NDARRAY_COMMON_H_
#define __ST_PPL_KERNEL_ARM_SERVER_PAD_NEON_PAD_NDARRAY_COMMON_H_
...
inline void pad_ndarray_last_dim_pad_begin_common(
    const eT *src,
...
template <typename eT, pad_mode_type_t _mode>
inline void pad_ndarray_last_dim_pad_end_common(
    const eT *src,
    const int64_t pad_begin,
...
template <typename eT, pad_mode_type_t _mode>
inline void pad_ndarray_begin_end_common(
    const eT *src,
    const int64_t pad_begin,
    const int64_t pad_end,
    const int64_t input_length,
    ...
    eT *dst)
{
    ...
    if (dim_idx == src_shape->GetDimCount() - 1) { // last dim
        pad_ndarray_last_dim_pad_begin_common<eT, _mode>(src, pad_begin, src_shape->GetDim(dim_idx), constant_value, dst);
        memcpy(dst + pad_begin, src, src_shape->GetDim(dim_idx) * sizeof(eT));
        pad_ndarray_last_dim_pad_end_common<eT, _mode>(src, pad_begin, pad_end, src_shape->GetDim(dim_idx), constant_value, dst);
    } else {
        if (src_shape->GetDim(dim_idx) > 1 && !has_paralleled) {
            PRAGMA_OMP_PARALLEL_FOR()
            for (int64_t i = 0; i < src_shape->GetDim(dim_idx); i++) {
                pad_ndarray_recursive_common<eT, _mode>(
                    src_shape,
                    dst_shape,
        ...
        pad_ndarray_begin_end_common<eT, _mode>(src, pad_begin, pad_end, src_shape->GetDim(dim_idx), stride_out[dim_idx], constant_value, dst);
    }
    ...
    eT *dst)
{
    ...
    return pad_ndarray_recursive_common<eT, _mode>(
        src_shape,
        dst_shape,
        src,
        start_pads,
...
#endif // __ST_PPL_KERNEL_ARM_SERVER_PAD_NEON_PAD_NDARRAY_COMMON_H_