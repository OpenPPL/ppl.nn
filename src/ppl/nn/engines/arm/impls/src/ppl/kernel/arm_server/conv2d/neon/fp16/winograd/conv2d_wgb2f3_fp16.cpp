...
std::vector<int64_t>  conv2d_wgb2f3_fp16_offline_manager::get_schedule_param() const
{
    std::vector<int64_t> sp = { sched_param_.ic_seg, sched_param_.oc_seg, sched_param_.tile_seg };
    return sp;
}

ppl::common::RetCode conv2d_wgb2f3_fp16_offline_manager::set_schedule_param(const std::vector<int64_t>& sp)
{
    if (sp.size() != 3) {
...
ppl::common::RetCode conv2d_wgb2f3_fp16_offline_manager::pick_best_schedule_param(
    ...
    double &run_time)
{
    ...
    auto conv_exe = conv2d_wgb2f3_fp16_runtime_executor(&param_, cvt_filter_, cvt_bias_, sched_param_);
...
ppl::common::RetCode conv2d_wgb2f3_fp16_offline_manager::try_fuse(conv_fuse_flag_t fuse_type)
{
    return ((fuse_type | conv_fuse_flag::HSWISH) || (fuse_type | conv_fuse_flag::PRELU )) ?
        ppl::common::RC_UNSUPPORTED : ppl::common::RC_SUCCESS;
}

ppl::common::RetCode conv2d_wgb2f3_fp16_offline_manager::generate_cvt_weights_shapes(
...
ppl::common::RetCode conv2d_wgb2f3_fp16_offline_manager::generate_cvt_weights(
    const void *filter,
    const void *bias,
    void* new_filter,
    void* new_bias)
