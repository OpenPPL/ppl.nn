...
#ifndef __ST_PPL_KERNEL_ARM_SERVER_RELATION_NEON_RELATION_BROADCAST_COMMON_H_
#define __ST_PPL_KERNEL_ARM_SERVER_RELATION_NEON_RELATION_BROADCAST_COMMON_H_
...
template <typename eT, int32_t c_blk, relation_op_type_t op_type>
inline void relation_broadcast_lastdim_no_broadcast_common(
    const eT *src0,
    const eT *src1,
    ...
    if(!c0_broadcast && !c1_broadcast){
        if (parallel){
            ...
            for (int64_t i = 0; i < length; i++)
            {
                ...
                vecType v_dst  = relation_vector_kernel<vecType, op_type>(v_src0, v_src1);
                pack_one(v_dst, dst + i * c_blk);
            }
        }else{
            for (int64_t i = 0; i < length; i++) {
                vecType v_src0 = vld<eT, eN>(src0 + i * c_blk);
...
template <typename eT, int32_t c_blk, relation_op_type_t op_type>
inline void relation_broadcast_lastdim_broadcast0_common(
    const eT *src0,
    const eT *src1,
    const int64_t length,
    const bool c0_broadcast,
...
template <typename eT, int32_t c_blk, relation_op_type_t op_type>
inline void relation_broadcast_lastdim_broadcast1_common(
    const eT *src0,
    const eT *src1,
    const int64_t length,
    const bool c0_broadcast,
...
template <typename eT, int32_t c_blk, relation_op_type_t op_type>
static ppl::common::RetCode relation_broadcast_recursive_common(
    ...
    uint8_t *dst)
{
    ...
    if (dim_idx == dim_count - 1) {
        ...
        if (src0_shape[dim_idx] == src1_shape[dim_idx]) {
            relation_broadcast_lastdim_no_broadcast_common<eT, c_blk, op_type>(src0, src1, length, c0_broadcast, c1_broadcast, lastdim_parallel, dst);
        } else if (src0_shape[dim_idx] == 1) {
            relation_broadcast_lastdim_broadcast0_common<eT, c_blk, op_type>(src0, src1, length, c0_broadcast, c1_broadcast, lastdim_parallel, dst);
        } else {
            relation_broadcast_lastdim_broadcast1_common<eT, c_blk, op_type>(src0, src1, length, c0_broadcast, c1_broadcast, lastdim_parallel, dst);
        }
    } else {
       if (dim_idx == loop_config->depth_of_loop && loop_config->num_threads > 1) {
            PRAGMA_OMP_PARALLEL_FOR()
            for (int64_t i = 0; i < length; i++) {
                relation_broadcast_recursive_common<eT, c_blk, op_type>(
                    src0_shape,
                    src1_shape,
                    dst_shape,
                    src0 + i * inc0[dim_idx],
...
inline void relation_prepare_incs(
    const int64_t *src0_shape,
    const int64_t *src1_shape,
    const int64_t *dst_shape,
    const int64_t dim_count,
...
#endif // __ST_PPL_KERNEL_ARM_SERVER_RELATION_NEON_RELATION_BROADCAST_COMMON_H_
