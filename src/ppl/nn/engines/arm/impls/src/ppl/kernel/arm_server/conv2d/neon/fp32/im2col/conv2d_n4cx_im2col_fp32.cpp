...
ppl::common::RetCode conv2d_n4cx_im2col_fp32_offline_manager::pick_best_schedule_param(
    ...
    double &run_time)
{
    ...
    auto conv_exe = conv2d_n4cx_im2col_fp32_runtime_executor(&param_, cvt_filter_, cvt_bias_, sched_param_, ker_param_);
    ...
    for (auto m_blk : candidate_m_blk_list) {
        for (auto n_blk : candidate_n_blk_list) {
            for (auto k_blk : candidate_k_blk_list) {
                conv_exe.sched_param_.sgemm_m_block1 = m_blk;
                conv_exe.sched_param_.sgemm_n_block1 = n_blk;
                conv_exe.sched_param_.sgemm_k_block1 = k_blk;
                conv_exe.adjust_schedule_param();

                size_t new_size = conv_exe.cal_temp_buffer_size();
...
ppl::common::RetCode conv2d_n4cx_im2col_fp32_offline_manager::try_fuse(conv_fuse_flag_t fuse_type)
{
    return ((fuse_type | conv_fuse_flag::HSWISH) || (fuse_type | conv_fuse_flag::PRELU )) ?
        ppl::common::RC_UNSUPPORTED : ppl::common::RC_SUCCESS;
}

ppl::common::RetCode conv2d_n4cx_im2col_fp32_offline_manager::generate_cvt_weights_shapes(
