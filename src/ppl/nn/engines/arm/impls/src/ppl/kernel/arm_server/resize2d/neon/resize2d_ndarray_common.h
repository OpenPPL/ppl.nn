    ...
    eT *dst)
{
    ...
    for (int64_t ow = 0; ow < dst_w; ++ow) {
        ...
        if (iw < 0) {
            ...
            w0        = (int64_t)iw;
            w1        = w0 + (w0 < src_w - 1 ? 1 : 0);
            w1_lambda = iw - w0;
            w0_lambda = 1.0f - w1_lambda;
        }
        w0_tab[ow]        = w0;
        w1_tab[ow]        = w1;
        w0_lambda_tab[ow] = w0_lambda;
        w1_lambda_tab[ow] = w1_lambda;
    }
    ...
    for (int64_t ni = 0; ni < num_imgs; ++ni) {
        for (int64_t oh = 0; oh < dst_h; ++oh) {
            ...
            for (int64_t ow = 0; ow < dst_w; ++ow) {
                int64_t w0      = w0_tab[ow];
                int64_t w1      = w1_tab[ow];
                float w0_lambda = w0_lambda_tab[ow];
                float w1_lambda = w1_lambda_tab[ow];
    ...
    eT *dst)
{
    ...
    for (int64_t ni = 0; ni < num_imgs; ++ni) {
        for (int64_t oh = 0; oh < dst_h; ++oh) {
            ...
            eT *t_dst       = l_dst + oh * dst_w;
            const eT *t_src = l_src + ih * src_w;
            int64_t ow      = 0;
            for (; ow < dst_w - 8; ow += 8) {
...
#endif // __ST_PPL_KERNEL_ARM_SERVER_RESIZE2D_NEON_RESIZE2D_NDARRAY_COMMON_H_
