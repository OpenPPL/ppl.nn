    ...
    float *y)
{
    ...
    for (int64_t i = 0; i < unroll_body; i += unroll_n) {
        ...
        vst1q_f32(y + i + simd_w * 0, v_sigmoid_f32(v_src_0));
        vst1q_f32(y + i + simd_w * 1, v_sigmoid_f32(v_src_1));
        vst1q_f32(y + i + simd_w * 2, v_sigmoid_f32(v_src_2));
        vst1q_f32(y + i + simd_w * 3, v_sigmoid_f32(v_src_3));
    }

    int64_t i = unroll_body;
    for (; i + simd_w <= n_elem; i += simd_w) {
        const float32x4_t v_src = vld1q_f32(x + i);
        vst1q_f32(y + i, v_sigmoid_f32(v_src));
    }
    for (; i < n_elem; ++i) {
        const float32x4_t v_src = vdupq_n_f32(x[i]);
        y[i]                    = vgetq_lane_f32(v_sigmoid_f32(v_src), 0);
    }
    ...
    __fp16 *y)
{
    ...
    for (int64_t i = 0; i < unroll_body; i += unroll_n) {
        ...
        vst1_f16(y + i + simd_w * 0, vcvt_f16_f32(v_sigmoid_f32(v_src_0)));
        vst1_f16(y + i + simd_w * 1, vcvt_f16_f32(v_sigmoid_f32(v_src_1)));
        vst1_f16(y + i + simd_w * 2, vcvt_f16_f32(v_sigmoid_f32(v_src_2)));
        vst1_f16(y + i + simd_w * 3, vcvt_f16_f32(v_sigmoid_f32(v_src_3)));
    }

    int64_t i = unroll_body;
    for (; i + simd_w <= n_elem; i += simd_w) {
        const float32x4_t v_src = vcvt_f32_f16(vld1_f16(x + i));
        vst1_f16(y + i, vcvt_f16_f32(v_sigmoid_f32(v_src)));
    }
    for (; i < n_elem; ++i) {
        const float32x4_t v_src = vdupq_n_f32((float)x[i]);
        y[i]                    = (__fp16)vgetq_lane_f32(v_sigmoid_f32(v_src), 0);
    }
