    ...
    eT *dst)
{
    ...
    if (dst_concat_dim * inner_dim * sizeof(eT) < 16 && num_src >= num_threads) { // when has small inner dims(usually occured when fixed_axis == ndims - 1), use memcpy_neon to replace memcpy & change index calculating method
