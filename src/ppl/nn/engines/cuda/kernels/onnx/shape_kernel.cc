...
ppl::common::RetCode ShapeKernel::DoExecute(KernelExecContext* ctx) {
    ...
    std::vector<int64_t> shape_host(shape->GetShape()->CalcElementsIncludingPadding());
    for (size_t i = 0; i < data->GetShape()->GetRealDimCount(); i++) {
        shape_host[i] = data->GetShape()->GetDim(i);
    }
    cudaMemcpyAsync(shape->GetBufferPtr(), shape_host.data(), shape->GetShape()->CalcBytesIncludingPadding(),
