...
#include "ppl/nn/engines/cuda/kernels/onnx/instance_normalization_kernel.h"

#include "cudakernel/nn/instance_normalization.h"

namespace ppl { namespace nn { namespace cuda {

ppl::common::RetCode InstanceNormalizationKernel::DoExecute(KernelExecContext* ctx) {
    ...
    ppl::common::RetCode status = PPLCUDAInstanceNormalizationForwardImp(
        GetStream(), input->GetShape(), input->GetBufferPtr(), scale->GetShape(), scale->GetBufferPtr(),
        ctx->GetInput<TensorImpl>(2)->GetBufferPtr(), 
        output->GetShape(), output->GetBufferPtr(), param_->param.epsilon,
