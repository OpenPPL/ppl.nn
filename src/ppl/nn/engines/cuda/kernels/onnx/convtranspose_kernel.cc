...
ppl::common::RetCode ConvTransposeKernel::DoExecute(KernelExecContext* ctx) {
    ...
    status = PPLCUDAConvTransposeForward(
                    GetCudaDevice()->GetDeviceId(), GetStream(), module, X->GetShape(), X->GetBufferPtr(), W->GetBufferPtr(), b_data,
                    Y->GetShape(), Y->GetBufferPtr(),
                    &param_->param, param_->extra_param.algo_info, temp_fuse_param, tmp_buffer);
