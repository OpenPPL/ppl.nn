...
#include "ppl/nn/engines/cuda/kernels/onnx/gather_elements_kernel.h"
...
RetCode GatherElementsOp::Init(const OptKernelOptions& options) {
...
GatherElementsOp::GatherElementsOp(const ir::Node* node) : CudaOptKernel(node) {
    ...
    infer_dims_func_ = [this](InputOutputInfo* info) -> RetCode {
        ...
        if (indices->IsScalar()) {
            ...
            output->Reshape(indices->GetDims(), indices->GetDimCount());
        }
        return RC_SUCCESS;
    };
}

RetCode GatherElementsOp::Finalize(const OptKernelOptions& options) {
...
KernelImpl* GatherElementsOp::CreateKernelImpl() const {
    return CreateKernelImplWithParam<GatherElementsKernel>(&param_);
}

#ifdef PPLNN_ENABLE_PMX_MODEL
    ppl::common::RetCode GatherElementsOp::SerializeData(const pmx::SerializationContext&, utils::DataStream* ds) const {
    ...
    ppl::common::RetCode GatherElementsOp::DeserializeData(const pmx::DeserializationContext&, const void* base, uint64_t size) {
