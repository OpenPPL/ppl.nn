...
RetCode XorOp::Init(const OptKernelOptions& options) {
    return RC_SUCCESS;
}

XorOp::XorOp(const ir::Node* node) : CudaOptKernel(node) {
...
RetCode XorOp::Finalize(const OptKernelOptions& options) {
...
KernelImpl* XorOp::CreateKernelImpl() const {
    return CreateKernelImplWithoutParam<XorKernel>();
}
