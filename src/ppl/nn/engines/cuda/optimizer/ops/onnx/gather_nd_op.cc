...
GatherNDOp::GatherNDOp(const ir::Node* node) : CudaOptKernel(node) {
    ...
    infer_dims_func_ = [this](InputOutputInfo* info) -> RetCode {
        return onnx::ReshapeGatherND(info, &param_);
    };
}
...
#ifdef PPLNN_ENABLE_PMX_MODEL
    ppl::common::RetCode GatherNDOp::SerializeData(const pmx::SerializationContext&, utils::DataStream* ds) const {
        flatbuffers::FlatBufferBuilder builder;
        auto fb_param = pmx::onnx::SerializeGatherNDParam(param_, &builder);
        auto fb_op_param = pmx::onnx::CreateOpParam(builder, pmx::onnx::OpParamType_GatherNDParam, fb_param.Union());
        pmx::onnx::FinishOpParamBuffer(builder, fb_op_param);
        return ds->Write(builder.GetBufferPointer(), builder.GetSize());
    }
    ppl::common::RetCode GatherNDOp::DeserializeData(const pmx::DeserializationContext&, const void* base, uint64_t size) {
        auto fb_op_param = pmx::onnx::GetOpParam(base);
        auto fb_argmax_param = fb_op_param->value_as_GatherNDParam();
        pmx::onnx::DeserializeGatherNDParam(*fb_argmax_param, &param_);
