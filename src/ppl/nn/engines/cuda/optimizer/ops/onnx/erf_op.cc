...
RetCode ErfOp::Init(const OptKernelOptions& options) {
    return RC_SUCCESS;
}

ErfOp::ErfOp(const ir::Node* node) : CudaOptKernel(node) {
...
RetCode ErfOp::Finalize(const OptKernelOptions& options) {
