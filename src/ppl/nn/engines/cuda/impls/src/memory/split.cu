        ...
        case sizeof(TYPE): {                                                                                                                                            \
            for (int i = 0; i < num_outputs; i++) {                                                                                                                     \
                ...
                if (split_size_with_axis < memcpy_threshold) {                                                                                                          \
                    ...
                    ppl_cukernel_split_ndarray<<<grid_size, block_size, 0, stream>>>(out_elems, split_size_fast, split_axis_length,                                     \
                        out_split_axis_size_fast, offset_split_axis, input_ptr, out_ptr);                                                                               \
                } else {                                                                                                                                                \
