...
template <typename T>
__device__ __inline__ T ppl_scalar_exp(const T &in_val)
{
    return exp(in_val);
}

template <>
__device__ __inline__ half ppl_scalar_exp<half>(const half &in_val)
{
...
template <typename T>
__global__ void ppl_cukernel_exp_ndarray(const uint64_t num_elems,
                                         const T *input,
                                         T *output)
{
#if __CUDA_ARCH__ >= 600 && __CUDACC_VER_MAJOR__ >= 9
    ...
    if (index >= num_elems)
        return;
    output[index] = ppl_scalar_exp<T>(input[index]);
#endif
}

template <typename T>
__global__ void ppl_cukernel_exp_nhwc(const uint64_t num_elems,
                                      int channels,
                                      int pad_channels,
                                      int chw,
                                      int hw,
    ...
    void *output)
{
    ...
    if (output_shape->GetDataFormat() == ppl::common::DATAFORMAT_NDARRAY) {
        ...
        if (output_shape->GetDataType() == ppl::common::DATATYPE_FLOAT32) {
            ppl_cukernel_exp_ndarray<float><<<grid_size, block_size, 0, stream>>>(num_elems,
                                                                                  (const float *)input,
                                                                                  (float *)output);
        } else if (output_shape->GetDataType() == ppl::common::DATATYPE_FLOAT16) {
            ppl_cukernel_exp_ndarray<half><<<grid_size, block_size, 0, stream>>>(num_elems,
                                                                                 (const half *)input,
                                                                                 (half *)output);
        ...
        grid_size.z = batch;
        if (output_shape->GetDataType() == ppl::common::DATATYPE_FLOAT32) {
            ppl_cukernel_exp_nhwc<float><<<grid_size, block_size, 0, stream>>>(
