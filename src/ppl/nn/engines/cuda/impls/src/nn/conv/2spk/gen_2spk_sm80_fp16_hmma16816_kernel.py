...
class KernelInfo:
    def __init__(self, path, flt_size, s_size, k_num, cta_y_num, cta_x_num, warp_y, warp_x, buf_size):
        ...
        self.sAv4_size = warp_y / self.INT4_TO_4HALF2
        self.sBv4_size = warp_x / self.INT4_TO_4HALF2

        self.dAv4_size = (self.cta_y * self.k_size * 1.0) / (self.INT4_TO_4HALF2 * self.cta_size)
        self.dBv4_size = (self.cta_x * self.k_size * 1.0) / (self.INT4_TO_4HALF2 * self.cta_size)
    ...
    def GetSMemUsage(self):
        ...
        sm_a_v4 = self.cta_y * self.k_size * self.buf_size / self.INT4_TO_4HALF2
        sm_b_v4 = self.cta_x * self.k_size * self.buf_size / self.INT4_TO_4HALF2
        sm_c_v4 = self.cta_y * self.cta_x / self.INT4_TO_4HALF2
    ...
    def GetRegUsage(self):
        ...
        reg_c_v4 = CeilDiv(self.cta_y * self.cta_x,  (self.HALF_SIZE * self.INT4_TO_4INT * self.thd_num_per_set))
    ...
    def GenKernel(self):
        ...
        if self.k_num == 2:
            f.write("#define REDUCE(_h2R)            REDUCE_HALF2_1x4(_h2R)\n\n")
        ...
        f.write("#define READ_sAv1(_A, _sm_base_v1, _sAv1_read)          READ_sUv1_K2_2x%d(_A, _sm_base_v1, _sAv1_read)\n"   % (self.warp_y / self.MMA_Y_HALF / 2))
        f.write("#define READ_sBv1(_B, _sm_base_v1, _sBv1_read)          READ_sUv1_K2_1x%d(_B, _sm_base_v1, _sBv1_read)\n\n" % (self.warp_x / self.MMA_X))
        ...
        if self.buf_size <= 2:
            if self.flt_size == "f1" or self.flt_size == "fs":
                f.write("#define LOAD_dAv4(_regA, _dA, _dAv4_off, _in_c_v8_id, _in_hw_valid)     LOAD_dAv4_SIZE%s(_regA, _dA, _dAv4_off, _in_c_v8_id, _in_hw_valid)\n" % self.GetSizeString(self.dAv4_size))
                f.write("#define WRITE_sAv4(_sm_base_v4, _sm_off, _reg)                          WRITE_sUv4_SIZE%s(_sm_base_v4, _sm_off, _reg)\n\n" % self.GetSizeString(self.dAv4_size))

                f.write("#define LOAD_dBv4(_regB, _dB, _dBv4_off, _flt_c_v8_id, _flt_n_valid)    LOAD_dBv4_SIZE%s(_regB, _dB, _dBv4_off, _flt_c_v8_id, _flt_n_valid)\n" % self.GetSizeString(self.dBv4_size))
                ...
                f.write("#define LOAD_dAv4(_regA, _dA, _dAv4_off, _in_c_v8_id, _flt_hw_bid)      LOAD_dAv4_SIZE%s(_regA, _dA, _dAv4_off, _in_c_v8_id, _flt_hw_bid)\n" % self.GetSizeString(self.dAv4_size))
                f.write("#define WRITE_sAv4(_sm_base_v4, _sm_off, _reg)                          WRITE_sUv4_SIZE%s(_sm_base_v4, _sm_off, _reg)\n\n" % self.GetSizeString(self.dAv4_size))
        ...
        f.write("#include \"2spk/fp16/main_body16816.h\"\n\n")
...
class LutSourceFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#include  \"2spk/sm80/fp16/hmma16816/%s_lut_kernels.h\"\n\n" % flt_size)
    ...
    def AppendKernel(self, fname):
        self.f.write("#include \"2spk/sm80/fp16/hmma16816/%s\"\n" % fname)
...
class SpkSourceFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#include  \"2spk/sm80/fp16/hmma16816/%s_spk_kernels.h\"\n\n" % flt_size)
...
class LutHeaderFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#ifndef __PPLCUDA_2SPK_SM80_FP16_HMMA16816_%s_LUT_KERNELS_H__\n" % flt_size.upper())
        self.f.write("#define __PPLCUDA_2SPK_SM80_FP16_HMMA16816_%s_LUT_KERNELS_H__\n" % flt_size.upper())
...
class SpkHeaderFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#ifndef __PPLCUDA_2SPK_SM80_FP16_HMMA16816_%s_SPK_KERNELS_H__\n" % flt_size.upper())
        self.f.write("#define __PPLCUDA_2SPK_SM80_FP16_HMMA16816_%s_SPK_KERNELS_H__\n" % flt_size.upper())
...
class InitFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("void Initialize2spkSM80FP16Hmma16816Conv%sKernelContainer(std::vector<kernel_info_t> & kernel_container)\n{\n" % self.flt_size.upper())
                            ...
                            for cta_y_num in [1, 2, 4]:
                                for cta_x_num in [1, 2, 4]:
                                    ...
                                    if cta_y_num == 4 and cta_x_num == 4:
                                        continue
                                    if cta_y_num * cta_x_num * k_num == 1 and s_size * k_num == 64:
                                        continue
                                    if cta_y_num * cta_x_num * k_num <= 2 and s_size * k_num == 128:
                                        continue
                                    if cta_y_num * cta_x_num * k_num <= 4 and s_size * k_num == 256:
                                        continue
