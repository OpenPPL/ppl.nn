...
__global__ void pad_kernel(const __half *input, __half *output, int h, int w, int pad_w)
{
    ...
    if (in_range)
        output[out_h * pad_w + out_w] = value;
}
...
#undef FAKE_GEMM_PARAM
    ...
    for (int64_t b = 0; b < batch / n_parallel_imgs; ++b) {
            ...
            columns);
        {
            ...
            grid.z = group;
            transpose_with_pad<__half><<<grid, threads, 0, stream>>>(columns, trans_columns, group, col_trans_in_h, col_trans_in_w, col_trans_out_h, col_trans_out_w);
        }
        ...
        for (int g = 0; g < group; g++) {
            ...
            PPLCUDAGemmForwardImp(
                device_prop, stream, function, &shape_a, tmp_a, &shape_b, tmp_b, tmp_bias, &shape_c, tmp_output, gemm_param, temp_buffer, fuse_param, algo_param);
        }
    }
