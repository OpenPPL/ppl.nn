...
class KernelInfo:
    def __init__(self, path, flt_size, s_size, k_num, cta_y_num, cta_x_num, warp_y, warp_x, buf_size):
        ...
        self.kname = "nv2spkSm80Int8Conv_imma16832_nhwc_" + self.flt_size + self.kconfig
    ...
    def GenKernel(self):
        ...
        f.write("#include \"2spk/int8/imma16832_macros.h\"\n\n")
        ...
        if self.buf_size <= 2:
            ...
            if self.flt_size == "f1" or self.flt_size == "fs":
                f.write("#define LOAD_dAv4(_sAv4, _sAv4_off, _dA, _dAv4_off, _in_c_v16_id, _in_hw_valid)     LOAD_dAv4_SIZE%s(_sAv4, _sAv4_off, _dA, _dAv4_off, _in_c_v16_id, _in_hw_valid)\n" % self.GetSizeString(self.dAv4_size))
                f.write("#define LOAD_dBv4(_sBv4, _sBv4_off, _dB, _dBv4_off, _flt_c_v16_id, _flt_n_valid)    LOAD_dBv4_SIZE%s(_sBv4, _sBv4_off, _dB, _dBv4_off, _flt_c_v16_id, _flt_n_valid)\n\n" % self.GetSizeString(self.dBv4_size))

                f.write("#define FLT_SIZE1\n\n")

            elif self.flt_size == "f3":
                f.write("#define LOAD_dAv4(_sAv4, _sAv4_off, _dA, _dAv4_off, _in_c_v16_id, _flt_hw_bid)      LOAD_dAv4_SIZE%s(_sAv4, _sAv4_off, _dA, _dAv4_off, _in_c_v16_id, _flt_hw_bid)\n" % self.GetSizeString(self.dAv4_size))
                f.write("#define LOAD_dBv4(_sBv4, _sBv4_off, _dB, _dBv4_off, _flt_c_v16_id, _flt_n_valid)    LOAD_dBv4_SIZE%s(_sBv4, _sBv4_off, _dB, _dBv4_off, _flt_c_v16_id, _flt_n_valid)\n\n" % self.GetSizeString(self.dBv4_size))
        ...
        f.write("#include \"2spk/int8/main_body16832.h\"\n\n")
...
class LutSourceFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#include  \"2spk/sm80/int8/imma16832/%s_lut_kernels.h\"\n\n" % flt_size)
    ...
    def AppendKernel(self, fname):
        self.f.write("#include \"2spk/sm80/int8/imma16832/%s\"\n" % fname)
...
class SpkSourceFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#include  \"2spk/sm80/int8/imma16832/%s_spk_kernels.h\"\n\n" % flt_size)
...
class LutHeaderFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#ifndef __PPLCUDA_2SPK_SM80_INT8_IMMA16832_%s_LUT_KERNELS_H__\n" % flt_size.upper())
        self.f.write("#define __PPLCUDA_2SPK_SM80_INT8_IMMA16832_%s_LUT_KERNELS_H__\n" % flt_size.upper())
...
class SpkHeaderFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#ifndef __PPLCUDA_2SPK_SM80_INT8_IMMA16832_%s_SPK_KERNELS_H__\n" % flt_size.upper())
        self.f.write("#define __PPLCUDA_2SPK_SM80_INT8_IMMA16832_%s_SPK_KERNELS_H__\n" % flt_size.upper())
...
class InitFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("void Initialize2spkSM80Int8Imma16832Conv%sKernelContainer(std::vector<kernel_info_t> & kernel_container)\n{\n" % self.flt_size.upper())
                            ...
                            for cta_y_num in [1, 2, 4]:
                                for cta_x_num in [1, 2, 4]:
                                    ...
                                    if cta_y_num * cta_x_num * k_num <= 2 and s_size * k_num == 256:
                                        continue
                                    if cta_y_num * cta_x_num * k_num <= 4 and s_size * k_num == 512:
                                        continue
