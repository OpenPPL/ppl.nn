...
#pragma unroll
...
#else
    uint smem_row_write_off = ((set_widx * TILE_N_V8_PER_WARP) ^ ((mma_idy % TILE_M_PER_MMA_HALF) / N_ROWS_PER_SMEM_ROW)
#endif
                       ) % SMEM_ROW_V8_SIZE;
    ...
    uint sRv4_read  = (mma_read_idy / TILE_M_PER_MMA_HALF) * TILE_N_V4_PER_CTA    * TILE_M_PER_MMA_HALF +
                      (mma_read_idy % TILE_M_PER_MMA_HALF) * TILE_N_V4_PER_CTA    +
                       smem_row_read_id  * SMEM_ROW_V4_SIZE     +
                    (((mma_read_idy % TILE_M_PER_MMA_HALF) / N_ROWS_PER_SMEM_ROW) ^ smem_inter_off)         * TILE_N_V4_PER_MMA +
                       smem_intra_off;
