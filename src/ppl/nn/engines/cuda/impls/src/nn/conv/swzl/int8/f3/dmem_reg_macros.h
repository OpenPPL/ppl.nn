...
#define LOAD_dBv4_SIZE_16TH(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0] += in_lut.idx[lut_id]; \
            \
            if(tid < ( CTA_SIZE_IN_THD / 16 ))  \
                _regB[0] = ( (_flt_hw_bid & in_hw_mask[0]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[0] ] : ZEROv4;\
        }

#define LOAD_dBv4_SIZE_8TH(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0] += in_lut.idx[lut_id]; \
            \
            if(tid < ( CTA_SIZE_IN_THD / 8 ))  \
...
#define LOAD_dBv4_SIZE_QTR(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0] += in_lut.idx[lut_id]; \
            \
            if(tid < ( CTA_SIZE_IN_THD / 4 ))  \
...
#define LOAD_dBv4_SIZE_HALF(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0] += in_lut.idx[lut_id]; \
            \
            if(tid < ( CTA_SIZE_IN_THD / 2 ))  \
...
#define LOAD_dBv4_SIZE1(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0] += in_lut.idx[lut_id]; \
            \
            _regB[0] = ( (_flt_hw_bid & in_hw_mask[0]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[0] ] : ZEROv4;\
...
#define LOAD_dBv4_SIZE2(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0] += in_lut.idx[lut_id]; \
            _dBv4_off[1] += in_lut.idx[lut_id]; \
            \
            _regB[0] = ( (_flt_hw_bid & in_hw_mask[0]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[0] ] : ZEROv4;\
            _regB[1] = ( (_flt_hw_bid & in_hw_mask[1]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[1] ] : ZEROv4;\
        }

#define LOAD_dBv4_SIZE4(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0] += in_lut.idx[lut_id]; \
            _dBv4_off[1] += in_lut.idx[lut_id]; \
            ...
            _regB[0] = ( (_flt_hw_bid & in_hw_mask[0]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[0] ] : ZEROv4;\
            _regB[1] = ( (_flt_hw_bid & in_hw_mask[1]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[1] ] : ZEROv4;\
            _regB[2] = ( (_flt_hw_bid & in_hw_mask[2]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[2] ] : ZEROv4;\
            _regB[3] = ( (_flt_hw_bid & in_hw_mask[3]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[3] ] : ZEROv4;\
        }

#define LOAD_dBv4_SIZE8(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0] += in_lut.idx[lut_id]; \
            _dBv4_off[1] += in_lut.idx[lut_id]; \
            _dBv4_off[2] += in_lut.idx[lut_id]; \
            ...
            _regB[2] = ( (_flt_hw_bid & in_hw_mask[2]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[2] ] : ZEROv4;\
            _regB[3] = ( (_flt_hw_bid & in_hw_mask[3]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[3] ] : ZEROv4;\
            _regB[4] = ( (_flt_hw_bid & in_hw_mask[4]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[4] ] : ZEROv4;\
            _regB[5] = ( (_flt_hw_bid & in_hw_mask[5]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[5] ] : ZEROv4;\
            _regB[6] = ( (_flt_hw_bid & in_hw_mask[6]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[6] ] : ZEROv4;\
            _regB[7] = ( (_flt_hw_bid & in_hw_mask[7]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[7] ] : ZEROv4;\
        }

#define LOAD_dBv4_SIZE16(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0]  += in_lut.idx[lut_id]; \
            _dBv4_off[1]  += in_lut.idx[lut_id]; \
            _dBv4_off[2]  += in_lut.idx[lut_id]; \
            ...
            _regB[6]  = ( (_flt_hw_bid & in_hw_mask[6])  && _in_c_v16_valid ) ? _dB[ _dBv4_off[6]  ] : ZEROv4;\
            _regB[7]  = ( (_flt_hw_bid & in_hw_mask[7])  && _in_c_v16_valid ) ? _dB[ _dBv4_off[7]  ] : ZEROv4;\
            _regB[8]  = ( (_flt_hw_bid & in_hw_mask[8])  && _in_c_v16_valid ) ? _dB[ _dBv4_off[8]  ] : ZEROv4;\
            _regB[9]  = ( (_flt_hw_bid & in_hw_mask[9])  && _in_c_v16_valid ) ? _dB[ _dBv4_off[9]  ] : ZEROv4;\
            _regB[10] = ( (_flt_hw_bid & in_hw_mask[10]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[10] ] : ZEROv4;\
            _regB[11] = ( (_flt_hw_bid & in_hw_mask[11]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[11] ] : ZEROv4;\
            _regB[12] = ( (_flt_hw_bid & in_hw_mask[12]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[12] ] : ZEROv4;\
            _regB[13] = ( (_flt_hw_bid & in_hw_mask[13]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[13] ] : ZEROv4;\
            _regB[14] = ( (_flt_hw_bid & in_hw_mask[14]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[14] ] : ZEROv4;\
            _regB[15] = ( (_flt_hw_bid & in_hw_mask[15]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[15] ] : ZEROv4;\
        }

#define LOAD_dBv4_SIZE32(_regB, _dB, _dBv4_off, _in_c_v16_valid, _flt_hw_bid) \
        { \
            _dBv4_off[0]  += in_lut.idx[lut_id]; \
            _dBv4_off[1]  += in_lut.idx[lut_id]; \
            _dBv4_off[2]  += in_lut.idx[lut_id]; \
            ...
            _regB[14] = ( (_flt_hw_bid & in_hw_mask[14]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[14] ] : ZEROv4;\
            _regB[15] = ( (_flt_hw_bid & in_hw_mask[15]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[15] ] : ZEROv4;\
            _regB[16] = ( (_flt_hw_bid & in_hw_mask[16]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[16] ] : ZEROv4;\
            _regB[17] = ( (_flt_hw_bid & in_hw_mask[17]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[17] ] : ZEROv4;\
            _regB[18] = ( (_flt_hw_bid & in_hw_mask[18]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[18] ] : ZEROv4;\
            _regB[19] = ( (_flt_hw_bid & in_hw_mask[19]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[19] ] : ZEROv4;\
            _regB[20] = ( (_flt_hw_bid & in_hw_mask[20]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[20] ] : ZEROv4;\
            _regB[21] = ( (_flt_hw_bid & in_hw_mask[21]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[21] ] : ZEROv4;\
            _regB[22] = ( (_flt_hw_bid & in_hw_mask[22]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[22] ] : ZEROv4;\
            _regB[23] = ( (_flt_hw_bid & in_hw_mask[23]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[23] ] : ZEROv4;\
            _regB[24] = ( (_flt_hw_bid & in_hw_mask[24]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[24] ] : ZEROv4;\
            _regB[25] = ( (_flt_hw_bid & in_hw_mask[25]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[25] ] : ZEROv4;\
            _regB[26] = ( (_flt_hw_bid & in_hw_mask[26]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[26] ] : ZEROv4;\
            _regB[27] = ( (_flt_hw_bid & in_hw_mask[27]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[27] ] : ZEROv4;\
            _regB[28] = ( (_flt_hw_bid & in_hw_mask[28]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[28] ] : ZEROv4;\
            _regB[29] = ( (_flt_hw_bid & in_hw_mask[29]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[29] ] : ZEROv4;\
            _regB[30] = ( (_flt_hw_bid & in_hw_mask[30]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[30] ] : ZEROv4;\
            _regB[31] = ( (_flt_hw_bid & in_hw_mask[31]) && _in_c_v16_valid ) ? _dB[ _dBv4_off[31] ] : ZEROv4;\
        }
