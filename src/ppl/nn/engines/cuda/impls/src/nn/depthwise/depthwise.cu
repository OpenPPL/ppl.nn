    ...
    void* output)
{
    ...
    for (uint32_t id = 0; id < func_vec.size(); id++) {
        ...
        for (int i = 0; i < times; i++) {
            ...
            func_vec[id].kernel_ptr<<<dim_grid, dim_block, 0, stream>>>((const half*)input, (const half*)filter, (const half*)bias, padc_fast, hw_fast, width_fast, in_height, in_width, kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, hole_h, hole_w, tile_height, tile_width, channels, paddingc, out_height, out_width, in_batch_stride, in_height_stride, in_width_stride, elems, (half*)output, fuse_param);
        }
    ...
    void* output)
{
    ...
    DivModFast width_fast(tile_width);
    dim3 dim_block(BLOCK_SIZE, 1, 1), dim_grid(DivUp(elems, BLOCK_SIZE), 1, 1);
    func_vec[kernel_id].kernel_ptr<<<dim_grid, dim_block, 0, stream>>>((const half*)input, (const half*)filter, (const half*)bias, padc_fast, hw_fast, width_fast, in_height, in_width, kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, hole_h, hole_w, tile_height, tile_width, channels, paddingc, out_height, out_width, in_batch_stride, in_height_stride, in_width_stride, elems, (half*)output, fuse_param);
}
