...
class KernelInfo:
    def __init__(self, path, flt_size, s_size, k_num, cta_y_num, cta_x_num, warp_y, warp_x, buf_size):
        ...
        self.kname = "nv2spkSm80Fp16Conv_hmma1688_nhwc_" + self.flt_size + self.kconfig
    ...
    def GetCompMem2Ratio(self):
        ...
        cycles_comp = self.CPI_HMMA1688 * ( (self.cta_y / self.MMA_Y) * (self.cta_x / self.MMA_X) * (self.k_size / self.MMA_K) / pb_num_per_cta) + self.HMMA_LATENCY + self.LDSM1_LATENCY
...
class LutSourceFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#include  \"2spk/sm80/fp16/hmma1688/%s_lut_kernels.h\"\n\n" % flt_size)
    ...
    def AppendKernel(self, fname):
        self.f.write("#include \"2spk/sm80/fp16/hmma1688/%s\"\n" % fname)
...
class SpkSourceFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#include  \"2spk/sm80/fp16/hmma1688/%s_spk_kernels.h\"\n\n" % flt_size)
...
class LutHeaderFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#ifndef __PPLCUDA_2SPK_SM80_FP16_HMMA1688_%s_LUT_KERNELS_H__\n" % flt_size.upper())
        self.f.write("#define __PPLCUDA_2SPK_SM80_FP16_HMMA1688_%s_LUT_KERNELS_H__\n" % flt_size.upper())
...
class SpkHeaderFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#ifndef __PPLCUDA_2SPK_SM80_FP16_HMMA1688_%s_SPK_KERNELS_H__\n" % flt_size.upper())
        self.f.write("#define __PPLCUDA_2SPK_SM80_FP16_HMMA1688_%s_SPK_KERNELS_H__\n" % flt_size.upper())
...
class InitFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("void Initialize2spkSM80FP16Hmma1688Conv%sKernelContainer(std::vector<kernel_info_t> & kernel_container)\n{\n" % self.flt_size.upper())
