...
class KernelInfo:
    def __init__(self, path, flt_size, s_size, k_num, cta_y_num, cta_x_num, warp_y, warp_x, buf_size):
        ...
        self.kname = "nvSwzlConv_imma8816_nhwc_" + self.flt_size + self.kconfig
        self.fname = self.flt_size + "/int8_swzl_"  + self.flt_size + self.kconfig + ".cu"
    ...
    def ExceedScope(self):
        ...
        sm_c_v4 = self.MMA_Y_HALF * self.cta_x / self.INT4_TO_4INT
    ...
    def GenKernel(self):
        ...
        f.write("#include \"int8_swzl/common/const_macros.h\"\n\n")
        f.write("#include \"int8_swzl/%s/bound_macros.h\"\n\n" % self.flt_size)
        f.write("#include \"int8_swzl/common/ldsm_macros.h\"\n\n")
        f.write("#include \"int8_swzl/%s/dmem_macros.h\"\n\n" % self.flt_size)
        f.write("#include \"int8_swzl/common/imma_macros.h\"\n\n")
        f.write("#include \"int8_swzl/common/reduce_macros.h\"\n\n")
        f.write("#include \"int8_swzl/common/smem_macros.h\"\n\n")
        ...
        f.write("#define WRITE_sRv2(_sm_base_v1, _sRv1_write_base, _C)   WRITE_sRv2_1x%d(_sm_base_v1, _sRv1_write_base, _C)\n\n" % (self.warp_x /self.MMA_X))
        ...
        f.write("#include \"int8_swzl/common/quant_macros.h\"\n\n")
        f.write("#include \"int8_swzl/common/output_macros.h\"\n\n")

        f.write("#include \"int8_swzl/common/main_body.h\"\n\n")

        f.write("#include \"int8_swzl/common/uni_undefs.h\"\n\n")
...
class LutSourceFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#include  \"int8_swzl/%s_lut_kernels.h\"\n\n" % flt_size)
    ...
    def AppendKernel(self, fname):
        self.f.write("#include \"int8_swzl/%s\"\n" % fname)
...
class SpkSourceFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#include  \"int8_swzl/%s_spk_kernels.h\"\n\n" % flt_size)
...
class LutHeaderFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#ifndef __PPLCUDA_INT8_SWZL_%s_LUT_KERNELS_H__\n" % flt_size.upper())
        self.f.write("#define __PPLCUDA_INT8_SWZL_%s_LUT_KERNELS_H__\n" % flt_size.upper())
...
class SpkHeaderFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("#ifndef __PPLCUDA_INT8_SWZL_%s_SPK_KERNELS_H__\n" % flt_size.upper())
        self.f.write("#define __PPLCUDA_INT8_SWZL_%s_SPK_KERNELS_H__\n" % flt_size.upper())
...
class InitFile:
    def __init__(self, path, flt_size):
        ...
        self.f.write("void InitializeInt8SwzlConv%sKernelContainer(std::vector<kernel_info_t> & kernel_container)\n{\n" % self.flt_size.upper())
                        ...
                        #for warp_x in [8, 16, 32, 64, 128]:
                            for cta_y_num in [1, 2, 4]:
                                for cta_x_num in [1, 2, 4]:
                                    if (warp_y*warp_x + warp_y*16*2/4 +warp_x*16*2/4)/32 >=255:
                                        continue
                                    if warp_y == 128 and warp_x == 64:
                                        continue
                                    if warp_y == 8 and warp_y == 8:
                                        continue
                                    if cta_y_num == 1 and cta_x_num == 1 and s_size == 128:
                                        continue;
