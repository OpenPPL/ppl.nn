    ...
    const int index /* index for debug only*/)
{
    ...
    T val = is_mode_avg
                ? (w1 * v1 + w2 * v2 + w3 * v3 + w4 * v4) // avg
                : max(max(max(w1 * v1, w2 * v2), w3 * v3), w4 * v4); // max
...
__global__ void roi_align_forward_cuda_kernel(
    ...
    const T* input,
    const T* rois,
    // batch_indices was introduced with onnx
        ...
        if (pool_mode == 1) {
            ...
            for (int iy = 0; iy < roi_bin_grid_h; iy++) {
                ...
                for (int ix = 0; ix < roi_bin_grid_w; ix++) {
                    ...
                    T val =
                        bilinear_interpolate(offset_input, height, width, y, x, false, index);
                    if (val > maxval) {
                        maxval = val;
                        // maxidx_y = y;
                        // maxidx_x = x;
                ...
                for (int ix = 0; ix < roi_bin_grid_w; ix++) {
                    const T x = roi_start_w + pw * bin_size_w +
                                static_cast<T>(ix + .5f) * bin_size_w /
                                    static_cast<T>(roi_bin_grid_w);
                    T val =
                        bilinear_interpolate(offset_input, height, width, y, x, true, index);
                    output_val += val;
                }
            }
...
// 2. roi_cols as an argument in onnx, but as an constant(5) in onnx
    ...
    ppl::nn::common::ROIAlignParam param)
{
    ...
    if (output_shape->GetDataType() == ppl::common::DATATYPE_FLOAT32) {
        roi_align_forward_cuda_kernel<float><<<grid_size, block_size, 0, stream>>>(num_elems, (const float*)input, (const float*)rois, (const int64_t*)batch_indices, (float*)output, param.output_height, param.output_width, param.spatial_scale, param.sampling_ratio, param.mode, roi_cols, aligned, channels, height, width);
    }
