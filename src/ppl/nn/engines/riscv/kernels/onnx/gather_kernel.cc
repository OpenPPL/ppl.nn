    ...
    if (data_format == ppl::common::DATAFORMAT_NDARRAY) {
        if (data_type == ppl::common::DATATYPE_FLOAT32) {
            return kernel::riscv::gather_ndarray_fp32(x->GetBufferPtr<float>(), y->GetBufferPtr<float>(),
                                                      real_indices.data(), outter_dim, gather_dim, inner_dim,
                                                      num_indices, indices_dim);
        } else if (data_type == ppl::common::DATATYPE_FLOAT16) {
            return kernel::riscv::gather_ndarray_fp16(x->GetBufferPtr<__fp16>(), y->GetBufferPtr<__fp16>(),
                                                      real_indices.data(), outter_dim, gather_dim, inner_dim,
                                                      num_indices, indices_dim);
        } else if (data_type == ppl::common::DATATYPE_INT64) {
            return kernel::riscv::gather_ndarray_int64(x->GetBufferPtr<int64_t>(), y->GetBufferPtr<int64_t>(),
                                                       real_indices.data(), outter_dim, gather_dim, inner_dim,
                                                       num_indices, indices_dim);
        } else {
