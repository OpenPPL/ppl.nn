...
ppl::common::RetCode AddKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (output_data_type == common::DATATYPE_FLOAT16) {
        return kernel::riscv::add_fp16(A->GetShape(), B->GetShape(), C->GetShape(), fuse_relu_,
        ...
        return kernel::riscv::add_fp32(A->GetShape(), B->GetShape(), C->GetShape(), fuse_relu_,
                                       A->GetBufferPtr<const float>(), B->GetBufferPtr<const float>(),
                                       C->GetBufferPtr<float>());
    } else if (output_data_type == common::DATATYPE_INT64) {
        return kernel::riscv::add_scalar_int64(A->GetShape(), B->GetShape(), C->GetShape(), fuse_relu_,
