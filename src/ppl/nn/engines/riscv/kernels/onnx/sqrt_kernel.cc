...
#include "ppl/nn/engines/riscv/kernels/onnx/sqrt_kernel.h"

#include "ppl/kernel/riscv/fp16/sqrt.h"
#include "ppl/kernel/riscv/fp32/sqrt.h"
...
ppl::common::RetCode SqrtKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (ppl::common::DATATYPE_FLOAT32 == data_type) {
        return ppl::kernel::riscv::sqrt_fp32(input->GetShape(), input->GetBufferPtr<float>(),
                                             output->GetBufferPtr<float>());
    } else if (ppl::common::DATATYPE_FLOAT16 == data_type) {
        return ppl::kernel::riscv::sqrt_fp16(input->GetShape(), input->GetBufferPtr<__fp16>(),
