...
#include "ppl/nn/engines/riscv/kernels/onnx/reduce_max_kernel.h"
...
ppl::common::RetCode ReduceMaxKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (data_type == ppl::common::DATATYPE_FLOAT16) {
        return kernel::riscv::reduce_max_fp16(data->GetBufferPtr<__fp16>(), reduced->GetBufferPtr<__fp16>(),
                                              data->GetShape(), reduced->GetShape(), fixed_axes.data(),
                                              fixed_axes.size());
    } else if (data_type == ppl::common::DATATYPE_FLOAT32) {
        return kernel::riscv::reduce_max_fp32(data->GetBufferPtr<float>(), reduced->GetBufferPtr<float>(),
                                              data->GetShape(), reduced->GetShape(), fixed_axes.data(),
                                              fixed_axes.size());
    } else if (data_type == ppl::common::DATATYPE_INT64) {
        return kernel::riscv::reduce_max_int64(data->GetBufferPtr<int64_t>(), reduced->GetBufferPtr<int64_t>(),
                                               data->GetShape(), reduced->GetShape(), fixed_axes.data(),
                                               fixed_axes.size());
    } else {
