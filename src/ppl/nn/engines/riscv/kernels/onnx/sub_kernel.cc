...
ppl::common::RetCode SubKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (output_data_type == common::DATATYPE_FLOAT16) {
        return kernel::riscv::sub_fp16(A->GetShape(), B->GetShape(), C->GetShape(), false,
        ...
        return kernel::riscv::sub_fp32(A->GetShape(), B->GetShape(), C->GetShape(), false,
                                       A->GetBufferPtr<const float>(), B->GetBufferPtr<const float>(),
                                       C->GetBufferPtr<float>());
    } else if (output_data_type == common::DATATYPE_INT64) {
        return kernel::riscv::sub_scalar_int64(A->GetShape(), B->GetShape(), C->GetShape(), false,
