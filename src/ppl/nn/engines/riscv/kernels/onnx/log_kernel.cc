...
#include "ppl/nn/engines/riscv/kernels/onnx/log_kernel.h"

#include "ppl/kernel/riscv/fp16/log.h"
#include "ppl/kernel/riscv/fp32/log.h"
...
ppl::common::RetCode LogKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (ppl::common::DATATYPE_FLOAT32 == data_type) {
        return ppl::kernel::riscv::log_fp32(input->GetShape(), input->GetBufferPtr<float>(),
                                            output->GetBufferPtr<float>());
    } else if (ppl::common::DATATYPE_FLOAT16 == data_type) {
        return ppl::kernel::riscv::log_fp16(input->GetShape(), input->GetBufferPtr<__fp16>(),
