...
ppl::common::RetCode MulKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (output_data_type == common::DATATYPE_FLOAT16) {
        return kernel::riscv::mul_fp16(A->GetShape(), B->GetShape(), C->GetShape(), fuse_relu_,
        ...
        return kernel::riscv::mul_fp32(A->GetShape(), B->GetShape(), C->GetShape(), fuse_relu_,
                                       A->GetBufferPtr<const float>(), B->GetBufferPtr<const float>(),
                                       C->GetBufferPtr<float>());
    } else if (output_data_type == common::DATATYPE_INT64) {
        return kernel::riscv::mul_scalar_int64(A->GetShape(), B->GetShape(), C->GetShape(), fuse_relu_,
                                               A->GetBufferPtr<const int64_t>(), B->GetBufferPtr<const int64_t>(),
                                               C->GetBufferPtr<int64_t>());
    } else {
        LOG(ERROR) << "unsupported data type: " << common::GetDataTypeStr(output_data_type);
    }
