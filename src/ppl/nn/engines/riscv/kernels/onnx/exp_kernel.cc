...
#include "ppl/nn/engines/riscv/kernels/onnx/exp_kernel.h"

#include "ppl/kernel/riscv/fp16/exp.h"
#include "ppl/kernel/riscv/fp32/exp.h"
...
ppl::common::RetCode ExpKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (ppl::common::DATATYPE_FLOAT32 == data_type) {
        return ppl::kernel::riscv::exp_fp32(input->GetShape(), input->GetBufferPtr<float>(),
                                            output->GetBufferPtr<float>());
    } else if (ppl::common::DATATYPE_FLOAT16 == data_type) {
        return ppl::kernel::riscv::exp_fp16(input->GetShape(), input->GetBufferPtr<__fp16>(),
