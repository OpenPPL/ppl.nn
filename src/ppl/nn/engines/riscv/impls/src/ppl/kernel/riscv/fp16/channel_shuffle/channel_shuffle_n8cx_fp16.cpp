...
inline void channel_shuffle_n8cx_pad_channel(const int64_t length, const int64_t c_eff, __fp16* dst) {
...
inline void channel_shuffle_n8cx_kernel(const __fp16* src[8], const int64_t& length, __fp16* dst) {
    const int64_t c_blk = 8;
    for (int64_t l = 0; l < length; ++l) {
        if (channels > 0)
            dst[l * c_blk + 0] = src[0][l * c_blk];
        if (channels > 1)
            dst[l * c_blk + 1] = src[1][l * c_blk];
        if (channels > 2)
            dst[l * c_blk + 2] = src[2][l * c_blk];
        if (channels > 3)
            dst[l * c_blk + 3] = src[3][l * c_blk];

        if (channels > 4)
            dst[l * c_blk + 4] = src[4][l * c_blk];
        if (channels > 5)
            dst[l * c_blk + 5] = src[5][l * c_blk];
        if (channels > 6)
            dst[l * c_blk + 6] = src[6][l * c_blk];
        if (channels > 7)
            dst[l * c_blk + 7] = src[7][l * c_blk];
    }
}
...
ppl::common::RetCode channel_shuffle_n8cx_fp16(const ppl::nn::TensorShape* src_shape, const __fp16* src,
    ...
    for (int64_t b = 0; b < batch; b++) {
        for (int64_t oc = 0; oc < channels; oc += c_blk) {
            ...
            __fp16* base_dst = dst + b * _3D * oc * _2D;
            const __fp16* base_src[8] = {0};
            ...
            if (oc_len_efficient == 8)
                channel_shuffle_n8cx_kernel<8>(base_src, _2D, base_dst);
            else if (oc_len_efficient == 7)
                channel_shuffle_n8cx_kernel<7>(base_src, _2D, base_dst);
            else if (oc_len_efficient == 6)
                channel_shuffle_n8cx_kernel<6>(base_src, _2D, base_dst);
            else if (oc_len_efficient == 5)
                channel_shuffle_n8cx_kernel<5>(base_src, _2D, base_dst);
            else if (oc_len_efficient == 4)
                channel_shuffle_n8cx_kernel<4>(base_src, _2D, base_dst);
            else if (oc_len_efficient == 3)
                channel_shuffle_n8cx_kernel<3>(base_src, _2D, base_dst);
            else if (oc_len_efficient == 2)
                channel_shuffle_n8cx_kernel<2>(base_src, _2D, base_dst);
            else if (oc_len_efficient == 1)
                channel_shuffle_n8cx_kernel<1>(base_src, _2D, base_dst);

            const bool pad_dst = oc + c_blk >= channels && pad_c != channels;
            if (pad_dst) {
                const int64_t round_dst_c = round(channels, c_blk);
                channel_shuffle_n8cx_pad_channel(_2D, channels - round_dst_c, dst + (b * pad_c + round_dst_c) * _2D);
            }
        }
    }
...
template <int32_t channels>
inline void channel_shuffle_n8cx_concat_split_kernel(const __fp16* src[8], const int64_t& length, __fp16* dst[8]) {
    const int64_t c_blk = 8;
    for (int64_t l = 0; l < length; l++) {
        if (channels > 0)
            dst[0][l * c_blk] = src[0][l * c_blk];
        if (channels > 1)
            dst[1][l * c_blk] = src[1][l * c_blk];
        if (channels > 2)
            dst[2][l * c_blk] = src[2][l * c_blk];
        if (channels > 3)
            dst[3][l * c_blk] = src[3][l * c_blk];
        if (channels > 4)
            dst[4][l * c_blk] = src[4][l * c_blk];
        if (channels > 5)
            dst[5][l * c_blk] = src[5][l * c_blk];
        if (channels > 6)
            dst[6][l * c_blk] = src[6][l * c_blk];
        if (channels > 7)
            dst[7][l * c_blk] = src[7][l * c_blk];
    }
}
...
ppl::common::RetCode channel_shuffle_n8cx_concat_split_fp16(const ppl::nn::TensorShape* src0_shape,
    ...
    for (int64_t b = 0; b < batch; b++) {
        for (int64_t oc = 0; oc < channels; oc += c_blk) {
            ...
            if (oc_len_efficient == 8)
                channel_shuffle_n8cx_concat_split_kernel<8>(base_src, inner_dims, base_dst);
            else if (oc_len_efficient == 7)
                channel_shuffle_n8cx_concat_split_kernel<7>(base_src, inner_dims, base_dst);
            else if (oc_len_efficient == 6)
                channel_shuffle_n8cx_concat_split_kernel<6>(base_src, inner_dims, base_dst);
            else if (oc_len_efficient == 5)
                channel_shuffle_n8cx_concat_split_kernel<5>(base_src, inner_dims, base_dst);
            else if (oc_len_efficient == 4)
                channel_shuffle_n8cx_concat_split_kernel<4>(base_src, inner_dims, base_dst);
            else if (oc_len_efficient == 3)
                channel_shuffle_n8cx_concat_split_kernel<3>(base_src, inner_dims, base_dst);
            else if (oc_len_efficient == 2)
                channel_shuffle_n8cx_concat_split_kernel<2>(base_src, inner_dims, base_dst);
            else if (oc_len_efficient == 1)
                channel_shuffle_n8cx_concat_split_kernel<1>(base_src, inner_dims, base_dst);
            ...
            if (pad_dst0) {
                ...
                channel_shuffle_n8cx_pad_channel(inner_dims, out_c1 - round_dst_c0,
                                                 dst0 + (b * padded_out_c1 + round_dst_c0) * inner_dims);
            }
            if (pad_dst1) {
                const int64_t round_dst_c1 = round(out_c2, c_blk);
                channel_shuffle_n8cx_pad_channel(inner_dims, out_c2 - round_dst_c1,
