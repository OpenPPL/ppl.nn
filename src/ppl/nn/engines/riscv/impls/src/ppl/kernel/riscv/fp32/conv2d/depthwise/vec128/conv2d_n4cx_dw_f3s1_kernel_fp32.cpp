...
void conv_dw_f3s1_h1w4_kernel_riscv_fp32(const float* src, const float* flt, const float* bias, float* dst,

                                         int64_t src_pad_w, int64_t dst_h, int64_t dst_w) {
    asm volatile(".equ           ATOM_W, %c[ATOM_W]      \n\t"
                 ...
                 : [ATOM_W] "i"(atom_w), [SRC] "r"(src), [FLT] "r"(flt), [DST] "r"(dst), [BIAS] "r"(bias),
                   [H_STRIDE] "r"(src_pad_w * 4 * 4), [DST_H] "r"(dst_h), [DST_W] "r"(dst_w)
