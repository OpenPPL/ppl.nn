    ...
    for (int64_t c = 0; c < pad_channels_div8; c++) {
        for (int64_t ht = 0; ht < num_h_tile; ht++) {
            for (int64_t wt = 0; wt < num_w_tile; wt++) {
                ...
                src_trans_kernel_func(src_pad + src_offset * 8, trans_mat, src_trans_pad_w * 8,
                                      src_trans_d + src_trans_offset * 8, src_trans_tile_stride);
            }
        }
    }
}
    ...
    for (int64_t c = 0; c < pad_num_outs_div8; c++) {
        for (int64_t ht = 0; ht < num_h_tile; ht++) {
            for (int64_t wt = 0; wt < num_w_tile; wt++) {
                ...
                dst_trans_kernel_func(dst_trans + dst_trans_offset * 8, bias + c * 8, trans_mat, dst_trans_tile_stride,
                                      dst + dst_offset * 8, dst_h_stride, dst_h_offset, dst_w_offset, dst_trans_h,
                                      dst_trans_w);
            }
        }
    }
...
void conv_wg_bxfxs1_pure_fp16(const __fp16* src, const __fp16* trans_mat_src, const __fp16* trans_mat_dst,
    ...
    for (int64_t h_dst_idx = 0; h_dst_idx < dst_pad_h; h_dst_idx += blk_dst_pad_h) {
        ...
        for (int64_t w_dst_idx = 0; w_dst_idx < dst_pad_w; w_dst_idx += blk_dst_pad_w) {
            ...
            conv_wg_s1_blocking_src_fp16(src, src_h, src_w, pad_channels, wgf, wgf, h_src_idx, w_src_idx,
