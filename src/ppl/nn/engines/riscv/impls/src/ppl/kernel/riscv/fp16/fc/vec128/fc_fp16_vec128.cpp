...
void fc_cvt_flt_riscv_fp16(const __fp16* flt, __fp16* flt_cvt,
...
void hgemm_n8chw_mxn8_riscv_fp16(const __fp16* src, const __fp16* flt, const __fp16* bias, __fp16* dst,
...
typedef void (*hgemm_n8chw_kernel_riscv_fp16_func)(const __fp16*, const __fp16*, const __fp16*, __fp16*, int32_t,
                                                   int32_t);
...
void fc_n8chw_riscv_fp16(const __fp16* src, const __fp16* flt, const __fp16* bias, __fp16* dst,
    ...
    for (int32_t oc = 0; oc < padded_num_outs; oc += 8) {
        int32_t bc = 0;
        for (; bc + 8 < batch; bc += 8) {
            hgemm_n8chw_mxn8_kernel_select[7](src + padded_channels * bc, flt + padded_channels * oc, bias + oc,
                                              dst + padded_num_outs * bc + oc,

                                              padded_channels, padded_num_outs);
        }
        if (bc < batch) {
            hgemm_n8chw_mxn8_kernel_select[batch - bc - 1](src + padded_channels * bc, flt + padded_channels * oc,
