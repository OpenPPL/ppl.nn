...
static void gemm_kernel_m16n7_n4cx_n4cx_fp32_vec128(const float* a, const float* b, float* c, const int64_t total_m,
                                                    const int64_t total_n, const int64_t total_k) {
    asm volatile(".equ            IS_FIRST, %c[IS_FIRST]     \n\t"
                 ".equ            ATOM_M, %c[ATOM_M]         \n\t"
                 ".equ            ATOM_N, %c[ATOM_N]         \n\t"
...
static void gemm_kernel_n4xc_n4xc_transpose_fp32_vec128(const float* src, float* dst, int64_t n, int64_t k,
...
void gemm_n4cx_n4cx_fp32_vec128(const float* a, const float* b, float* c, const int64_t total_m, const int64_t total_n,
    ...
    for (mi = 0; mi <= total_m - align_m; mi += align_m) {
        ...
        for (ni = 0; ni <= total_n - align_n; ni += align_n) {
            gemm_kernel_m16n7_n4cx_n4cx_fp32_vec128<align_m, align_n, first>(temp_a, temp_b, temp_c, total_m, total_n,
                                                                             total_k);
            temp_b += atom_k * align_n;
            temp_c += atom_m * align_n;
        }
        if (ni < total_n) {
            gemm_kernel_m16n7_n4cx_n4cx_fp32_vec128<align_m, left_n, first>(temp_a, temp_b, temp_c, total_m, total_n,
                                                                            total_k);
    ...
    if (mi < total_m) {
        ...
        const float* temp_b = b;
        for (ni = 0; ni <= total_n - align_n; ni += align_n) {
            gemm_kernel_m16n7_n4cx_n4cx_fp32_vec128<left_m, align_n, first>(temp_a, temp_b, temp_c, total_m, total_n,
                                                                            total_k);
            temp_b += atom_k * align_n;
            temp_c += atom_m * align_n;
        }
        if (ni < total_n) {
            gemm_kernel_m16n7_n4cx_n4cx_fp32_vec128<left_m, left_n, first>(temp_a, temp_b, temp_c, total_m, total_n,
                                                                           total_k);
        }
    }
}
