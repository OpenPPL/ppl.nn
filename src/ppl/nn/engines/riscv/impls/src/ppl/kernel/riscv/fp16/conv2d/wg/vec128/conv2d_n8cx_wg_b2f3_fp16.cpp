...
inline void wg_b2f3s1_src_trans_kernel(const __fp16* src_pad,
    ...
    asm volatile("mv             t0,     %[src]          \n\t"
                 ...
                 "vse.v          v3,     (t0)            \n\t"
                 :
                 : [src] "r"(src_pad), [dst] "r"(src_trans_d), [src_offset] "r"(src_pad_h_stride * 2),
                   [dst_offset] "r"(src_trans_wg_tile_stride * 2)
                 : "memory", "v0", "v1", "v2", "v3", "v16", "v17", "v18", "v19", "v20", "v21", "v22", "v23", "v24",
                   "v25", "v26", "v27", "v28", "v29", "v30", "v31", "t0", "t1", "t2", "t6");
}

inline void wg_b2f3s1_dst_trans_kernel(const __fp16* dst_trans, const __fp16* bias,
                                       const __fp16* trans_mat, // TODO: should be removed
    ...
    asm volatile("addi           t0,     x0,     8       \n\t"
                 ...
                 "addi           x0,     x0,     1       \n\t"
                 :
                 : [src] "r"(dst_trans), [dst] "r"(dst), [src_offset0] "r"(dst_trans_wg_tile_stride * 8),
                   [src_offset1] "r"(dst_trans_wg_tile_stride * 2), [dst_offset] "r"(dst_h_stride * 2),
...
ppl::common::RetCode conv2d_n8cx_wg_b2f3_fp16_runtime_executor::execute() {
    ...
    conv_shell_riscv_fp16<
        conv2d_n8cx_wg_bxfxs1_fp16_vec128_extra_param, 8, get_real_filter_size<2, 3>,
