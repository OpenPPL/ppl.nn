...
inline void wg_b4f3s1_src_trans_kernel(const float* src_pad,
    ...
    asm volatile("mv             t0,     %[mat]          \n\t"
                 ...
                 : [src] "r"(src_pad), [dst] "r"(src_trans_d), [mat] "r"(trans_mat), [tmp] "r"(tmp),
                   [src_offset] "r"(src_pad_h_stride * 4), [dst_offset] "r"(src_trans_wg_tile_stride * 4)
                 : "memory", "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v16", "v17", "v18", "v20", "v21", "v22", "v23",
                   "v24", "v25", "v30", "v31", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "s2");
}

inline void wg_b4f3s1_dst_trans_kernel(const float* dst_trans, const float* bias,
                                       const float* trans_mat, // TODO: should be removed
    ...
    asm volatile(
        ...
        : [src] "r"(dst_trans), [dst] "r"(dst), [mat] "r"(trans_mat), [bias] "r"(bias),
          [src_offset0] "r"(dst_trans_wg_tile_stride * 4), [dst_offset] "r"(dst_h_stride * 4),
          [src_offset1] "r"(dst_trans_wg_tile_stride * 24), [h_offset] "r"(dst_h_offset), [w_offset] "r"(dst_w_offset),
...
ppl::common::RetCode conv2d_n4cx_wg_b4f3_fp32_runtime_executor::execute() {
    ...
    conv2d_shell_fp32<
        conv2d_n4cx_wg_bxfxs1_fp32_vec128_extra_param, 4, conv2d_get_real_filter_size<4, 3>,
