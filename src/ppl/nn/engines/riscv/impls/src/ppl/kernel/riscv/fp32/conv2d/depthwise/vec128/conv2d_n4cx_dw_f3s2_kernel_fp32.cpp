...
void conv_dw_f3s2_h3w4_kernel_riscv_fp32(const float* src, const float* flt, const float* bias, float* dst,
    ...
    asm volatile(".equ           ATOM_H, %c[ATOM_H]      \n\t"
                 ...
                 : [ATOM_H] "i"(atom_h), [ATOM_W] "i"(atom_w), [SRC] "r"(src), [FLT] "r"(flt), [DST] "r"(dst),
                   [BIAS] "r"(bias), [H_STRIDE] "r"(src_pad_w * 4 * 4), [DST_H] "r"(dst_h), [DST_W] "r"(dst_w)
