...
size_t conv2d_n8cx_gemm_get_cvt_filter_size_fp16_vec128(int64_t num_outs, int64_t channels, int64_t kernel_h, int64_t kernel_w, int64_t group)
{
...
size_t conv2d_n8cx_gemm_get_temp_buffer_size_fp16_vec128(int64_t src_h, int64_t src_w, int64_t channels, int64_t num_outs, int64_t group, int64_t pad_h, int64_t pad_w, int64_t flt_h, int64_t flt_w, int64_t hole_h, int64_t hole_w, int64_t stride_h, int64_t stride_w,
...
uint64_t conv2d_n8cx_gemm_fp16_runtime_executor::cal_temp_buffer_size()
{
    size_t temp_buffer_size = conv2d_n8cx_gemm_get_temp_buffer_size_fp16_vec128(
        src_shape_->GetDim(2), src_shape_->GetDim(3), conv_param_->channels, conv_param_->num_output, conv_param_->group, conv_param_->pad_h, conv_param_->pad_w, conv_param_->kernel_h, conv_param_->kernel_w, conv_param_->dilation_h, conv_param_->dilation_w, conv_param_->stride_h, conv_param_->stride_w,
...
void sgemm_riscv_n8cx_per_group(const __fp16* src, const __fp16* filter, const __fp16* bias, __fp16* gemm_buffer, __fp16* dst,
        ...
        for (int64_t k = 0; k < K; k += k_blk) {
            ...
            for (int64_t n = 0; n < N; n += n_blk) {
                ...
                if (first) {
                    ...
                    sgemm_n8cx_tile_kernel(filter, gemm_src_loc, gemm_dst_loc + gemm_dst_stride, real_blk_m * atom_oc, real_blk_n / atom_oc, real_blk_k / atom_oc);
                } else {
                ...
                if (k + real_blk_k == K) {
                    ...
                    sgemm_riscv_n8cx_cvt_dst(gemm_dst_loc + gemm_dst_stride, bias + bias_offset, dst_offset, M, N, real_blk_m, real_blk_n, 1, atom_oc, m_loop, n_loop);
                }
...
void conv2d_n8cx_gemm_per_group_fp16_vec128(const __fp16* src, const __fp16* filter, const __fp16* bias, __fp16* temp_buffer, __fp16* dst,
                                            ...
                                            conv2d_n8cx_gemm_tunning_info tunning_info)
{
    ...
    if (flt_h == 1 && flt_w == 1 && pad_h == 0 && pad_w == 0 && stride_h == 1 && stride_w == 1 && hole_h == 1 &&
        ...
        im2col_riscv_n8cx_per_group(src, im2col_buf, padded_ic, src_h, src_w, flt_h, flt_w, pad_h, pad_w, stride_h, stride_w, hole_h, hole_w, dst_h, dst_w);
...
ppl::common::RetCode conv2d_n8cx_gemm_fp16_runtime_executor::execute()
{
    ...
    conv_shell_riscv_fp16<conv2d_n8cx_gemm_tunning_info, 8, conv2d_gemm_get_real_filter_size, conv2d_n8cx_gemm_per_group_fp16_vec128>(
                                             ...
                                             int64_t k_blk)
{
    ...
    for (int64_t g = 0; g < group; g++) {
        sgemm_riscv_n8cx_cvt_filter_per_group(
            filter + g * flt_size_per_group, filter_cvt + g * flt_cvt_size_per_group, num_outs_per_group, channels_per_group, flt_h, flt_w, num_outs_per_group / 8, channels_per_group * flt_h * flt_w * 8, padded_num_outs_per_group / 8, padded_channels_per_group * flt_h * flt_w * 8, m_blk, k_blk, 1, 8 * 8);
    }
}
...
ppl::common::RetCode conv2d_n8cx_gemm_fp16_offline_manager::gen_cvt_weights(const __fp16* filter, const __fp16* bias)
{
    ...
    // cvt filter
    {
        ...
        conv2d_n8cx_gemm_cvt_filter_fp16_vec128(filter, cvt_filter_, kernel_h, kernel_w, channels, num_output, num_group, tunning_param_.m_blk, tunning_param_.k_blk);
    }
