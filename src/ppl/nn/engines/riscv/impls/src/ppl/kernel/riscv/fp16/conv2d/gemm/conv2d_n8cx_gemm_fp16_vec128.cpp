...
size_t conv2d_n8cx_gemm_get_cvt_filter_size_fp16_vec128(int64_t num_outs, int64_t channels, int64_t kernel_h,
...
size_t conv2d_n8cx_gemm_get_temp_buffer_size_fp16_vec128(int64_t src_h, int64_t src_w, int64_t channels,
...
void sgemm_riscv_n8cx_cvt_src(const __fp16* src, __fp16* dst,
...
void sgemm_riscv_n8cx_cvt_dst(const __fp16* src, const __fp16* bias, __fp16* dst,
...
void im2col_riscv_n8cx_per_group(const __fp16* src, __fp16* dst,
...
void sgemm_riscv_n8cx_per_group(const __fp16* src, const __fp16* filter, const __fp16* bias, __fp16* gemm_buffer,
                                __fp16* dst,
    ...
    for (int64_t m = 0; m < M; m += m_blk) {
        ...
        for (int64_t k = 0; k < K; k += k_blk) {
            ...
            for (int64_t n = 0; n < N; n += n_blk) {
                ...
                if (first) {
                    auto sgemm_n8cx_tile_kernel = conv_gemm_select_xcto8c_kernel_fp16<8, true>(
                        real_blk_m * atom_oc, real_blk_n / atom_oc);
                    sgemm_n8cx_tile_kernel(filter, gemm_src_loc, gemm_dst_loc + gemm_dst_stride, real_blk_m * atom_oc,
                                           real_blk_n / atom_oc, real_blk_k / atom_oc);
                } else {
                    auto sgemm_n8cx_tile_kernel = conv_gemm_select_xcto8c_kernel_fp16<8, false>(
                        real_blk_m * atom_oc, real_blk_n / atom_oc);
                    sgemm_n8cx_tile_kernel(filter, gemm_src_loc, gemm_dst_loc + gemm_dst_stride, real_blk_m * atom_oc,
                                           real_blk_n / atom_oc, real_blk_k / atom_oc);
                }
                ...
                if (k + real_blk_k == K) {
                    ...
                    sgemm_riscv_n8cx_cvt_dst(gemm_dst_loc + gemm_dst_stride, bias + bias_offset, dst_offset, M, N,
...
void conv2d_n8cx_gemm_per_group_fp16_vec128(const __fp16* src, const __fp16* filter, const __fp16* bias,
    ...
    if (flt_h == 1 && flt_w == 1 && pad_h == 0 && pad_w == 0 && stride_h == 1 && stride_w == 1 && hole_h == 1 &&
        ...
        im2col_riscv_n8cx_per_group(src, im2col_buf, padded_ic, src_h, src_w, flt_h, flt_w, pad_h, pad_w, stride_h,
                                    stride_w, hole_h, hole_w, dst_h, dst_w);
        sgemm_riscv_n8cx_per_group(im2col_buf, filter, bias, gemm_buffer, dst, M, N, K, gemm_m_blk, gemm_n_blk,
                                   gemm_k_blk);
    }
}
...
ppl::common::RetCode conv2d_n8cx_gemm_fp16_runtime_executor::execute() {
    ...
    conv_shell_riscv_fp16<conv2d_n8cx_gemm_tunning_info, 8, conv2d_gemm_get_real_filter_size,
...
ppl::common::RetCode conv2d_n8cx_gemm_fp16_offline_manager::pick_best_tunning_param(const __fp16* src,
...
void sgemm_riscv_n8cx_cvt_filter_per_group(const __fp16* flt, __fp16* flt_cvt,
...
void conv2d_n8cx_gemm_cvt_filter_fp16_vec128(const __fp16* filter, __fp16* filter_cvt,
    ...
    for (int64_t g = 0; g < group; g++) {
        sgemm_riscv_n8cx_cvt_filter_per_group(
            ...
            channels_per_group, flt_h, flt_w, num_outs_per_group / 8, channels_per_group * flt_h * flt_w * 8,
            padded_num_outs_per_group / 8, padded_channels_per_group * flt_h * flt_w * 8, m_blk, k_blk, 1, 8 * 8);
    }
}

ppl::common::RetCode conv2d_n8cx_gemm_fp16_offline_manager::gen_cvt_weights(const __fp16* filter, const __fp16* bias) {
    ...
    {
        cvt_filter_size_ =
            conv2d_n8cx_gemm_get_cvt_filter_size_fp16_vec128(num_output, channels, kernel_h, kernel_w, num_group);
        cvt_filter_ = (__fp16*)allocator_->Alloc(cvt_filter_size_);
        conv2d_n8cx_gemm_cvt_filter_fp16_vec128(filter, cvt_filter_, kernel_h, kernel_w, channels, num_output,
