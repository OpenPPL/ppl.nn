    ...
    int64_t src_trans_wg_tile_stride)
{
    ...
    asm volatile(
        ...
        "addi           x0,     x0,         1           \n\t"
        :
        : [src] "r"(src_pad), [dst] "r"(src_trans_d), [mat] "r"(trans_mat), [tmp] "r"(tmp), [src_ofst] "r"(src_pad_h_stride * 4), [dst_ofst] "r"(src_trans_wg_tile_stride * 4)
    ...
    int64_t dst_trans_w)
{
    ...
    asm volatile(
        ...
        "addi           x0,         x0,         1           \n\t"
        :
        : [src] "r"(dst_trans), [dst] "r"(dst), [mat] "r"(trans_mat), [tmp] "r"(tmp), [bias] "r"(bias), [src_ofst0] "r"(dst_trans_wg_tile_stride * 32), [dst_ofst] "r"(dst_h_stride * 4), [src_ofst1] "r"(dst_trans_wg_tile_stride * 4), [h_ofst] "r"(dst_h_offset), [w_ofst] "r"(dst_w_offset), [dst_h] "r"(dst_trans_h), [dst_w] "r"(dst_trans_w)
...
ppl::common::RetCode conv2d_n4cx_wg_b6f3_fp32_runtime_executor::execute()
{
    ...
    conv2d_shell_fp32<
        ...
        4,
        conv2d_get_real_filter_size<6, 3>,
