...
void conv2d_n4cx_wg_b6f3_fp32_runtime_executor::adjust_tunning_param() {
...
ppl::common::RetCode conv2d_n4cx_wg_b6f3_fp32_runtime_executor::prepare() {
    if (!conv_param_ || !src_shape_ || !dst_shape_) {
        return ppl::common::RC_INVALID_VALUE;
    }
    adjust_tunning_param();
    LOG(DEBUG) << "n4cx wg b6f3: prepare";

    return ppl::common::RC_SUCCESS;
}

inline void wg_b6f3s1_src_trans_kernel(const float* src_pad,
    ...
    asm volatile("addi           t0,     x0,     4               \n\t"
                 ...
                 : [src] "r"(src_pad), [dst] "r"(src_trans_d), [mat] "r"(trans_mat), [tmp] "r"(tmp),
                   [src_ofst] "r"(src_pad_h_stride * 4), [dst_ofst] "r"(src_trans_wg_tile_stride * 4)
...
inline void wg_b6f3s1_dst_trans_kernel(const float* dst_trans, const float* bias,
                                       const float* trans_mat, // TODO: should be removed
                                       int64_t dst_trans_wg_tile_stride,

                                       float* dst, int64_t dst_h_stride, int64_t dst_h_offset, int64_t dst_w_offset,
    ...
    asm volatile("addi           t0,         x0,     4               \n\t"
                 ...
                 : [src] "r"(dst_trans), [dst] "r"(dst), [mat] "r"(trans_mat), [tmp] "r"(tmp), [bias] "r"(bias),
                   [src_ofst0] "r"(dst_trans_wg_tile_stride * 32), [dst_ofst] "r"(dst_h_stride * 4),
                   [src_ofst1] "r"(dst_trans_wg_tile_stride * 4), [h_ofst] "r"(dst_h_offset),
...
ppl::common::RetCode conv2d_n4cx_wg_b6f3_fp32_runtime_executor::execute() {
    const conv2d_common_param& cp = *conv_param_;

    LOG(DEBUG) << "n4cx wg b6f3: execute";
    if (src_ == nullptr || cvt_bias_ == nullptr || cvt_filter_ == nullptr || temp_buffer_ == nullptr ||
        dst_ == nullptr) {
        return ppl::common::RC_INVALID_VALUE;
    }

    const float trans_mat_src_[8] = {5.25f, 4.25f, 0.5f, 0.25f, 2.5f, 1.25f, 2.0f, 4.0f};
    const float trans_mat_dst_[5] = {32.0f, 16.0f, 8.0f, 4.0f, 2.0f};

    conv2d_shell_fp32<
        conv2d_n4cx_wg_bxfxs1_fp32_vec128_extra_param, 4, conv2d_get_real_filter_size<6, 3>,
        conv2d_conv_wg_bxfxs1_riscv_per_group_fp32<6, 3, wg_b6f3s1_src_trans_kernel, wg_b6f3s1_dst_trans_kernel>>(
