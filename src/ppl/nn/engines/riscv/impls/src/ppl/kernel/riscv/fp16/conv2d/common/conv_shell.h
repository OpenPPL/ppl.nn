...
template <typename T>
using conv_per_group_riscv_func_type = void (*)(
    const __fp16* src,
    const __fp16* filter,
    const __fp16* bias,
    __fp16* temp_buffer,
...
template <typename T,
          int64_t atom_ic,
          get_real_filter_size_func_type get_real_filter_size,
          conv_per_group_riscv_func_type<T> conv_per_group>
static void conv_shell_riscv_fp16(
    const __fp16* src,
    const __fp16* filter,
    const __fp16* bias,
    __fp16* temp_buffer,
    ...
    T tunning_info)
{
    ...
    if (group == 1) {
        ...
        for (int64_t i = 0; i < batch; i += 1) {
            auto src_per_batch_ptr = src + i * src_batch_stride;
            if (ic_per_gp % atom_ic != 0) {
                divide_src_8c_for_group(src_per_batch_ptr, src_h * src_w, ic_per_gp, group, src_div_loc);
