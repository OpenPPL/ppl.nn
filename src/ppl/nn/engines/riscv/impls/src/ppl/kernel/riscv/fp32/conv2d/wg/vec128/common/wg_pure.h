...
typedef void (*conv_wg_riscv_fp32_n4chw_src_trans_kernel_func_t)(const float* src_pad,
                                                                 const float* trans_mat, // TODO: should be removed
                                                                 int64_t src_pad_h_stride,

                                                                 float* src_trans_d, int64_t src_trans_wg_tile_stride);

typedef void (*conv_wg_riscv_fp32_n4chw_dst_trans_kernel_func_t)(const float* dst_trans, const float* bias,
                                                                 const float* trans_mat, // TODO: should be removed
                                                                 int64_t dst_trans_wg_tile_stride,

                                                                 float* dst, int64_t dst_h_stride, int64_t dst_h_offset,
                                                                 int64_t dst_w_offset, int64_t dst_trans_h,
                                                                 int64_t dst_trans_w);

template <int64_t wgb, int64_t wgf, conv_wg_riscv_fp32_n4chw_src_trans_kernel_func_t src_trans_kernel_func>
void conv2d_conv_wg_bxfxs1_src_blk_trans_fp32(const float* src_pad, const float* trans_mat, int64_t src_trans_pad_h,
                                              int64_t src_trans_pad_w, int64_t pad_channels, int64_t num_h_tile,
                                              int64_t num_w_tile, float* src_trans_d) {
    ...
    for (int64_t c = 0; c < pad_channels_div4; c++) {
        for (int64_t ht = 0; ht < num_h_tile; ht++) {
            for (int64_t wt = 0; wt < num_w_tile; wt++) {
                ...
                src_trans_kernel_func(src_pad + src_offset * C_BLK(), trans_mat, src_trans_pad_w * C_BLK(),
                                      src_trans_d + src_trans_offset * C_BLK(), src_trans_tile_stride);
            }
        }
    }
}

template <int64_t wgb, int64_t wgf, conv_wg_riscv_fp32_n4chw_dst_trans_kernel_func_t dst_trans_kernel_func>
void conv2d_conv_wg_bxfxs1_dst_blk_trans_fp32(const float* dst_trans, const float* trans_mat, const float* bias,
                                              ...
                                              float* dst, int64_t dst_h, int64_t dst_w) {
    int64_t tile_len = wgb + wgf - 1;
    ...
    for (int64_t c = 0; c < pad_num_outs_div4; c++) {
        for (int64_t ht = 0; ht < num_h_tile; ht++) {
            for (int64_t wt = 0; wt < num_w_tile; wt++) {
                ...
                dst_trans_kernel_func(dst_trans + dst_trans_offset * C_BLK(), bias + c * C_BLK(), trans_mat,
                                      dst_trans_tile_stride, dst + dst_offset * C_BLK(), dst_h_stride, dst_h_offset,
                                      dst_w_offset, dst_trans_h, dst_trans_w);
            }
        }
    }
}

static void conv2d_conv_wg_s1_blocking_src_fp32(const float* src, int64_t src_h, int64_t src_w, int64_t pad_channels,
                                                int64_t flt_h, int64_t flt_w,

                                                int64_t src_h_beg, int64_t src_w_beg, int64_t blk_src_h,
                                                int64_t blk_src_w,
    ...
    for (int64_t i = 0; i < pad_channels; i += C_BLK()) {
        ...
        for (; src_h_idx < src_h_end; src_h_idx += 1) {
            ...
            if (src_w_idx < src_w_end) {
                ...
                memcpy(src_pad_d, src_per_channels + src_h_idx * src_h_stride + src_w_idx * C_BLK(),
                       num_cpy * sizeof(float));
                src_pad_d += num_cpy;
                src_w_idx = src_w_end;
            }
...
void conv2d_conv_wg_bxfxs1_pure_fp32(const float* src, const float* trans_mat_src, const float* trans_mat_dst,
                                     int64_t src_h, int64_t src_w, int64_t channels, int64_t num_outs,
                                     int64_t padding_h, int64_t padding_w,

                                     int64_t blk_dst_h, int64_t blk_dst_w, int64_t blk_channels, int64_t blk_num_outs,

                                     const float* filter, const float* bias, float* temp_buffer, float* dst) {
    ...
    for (int64_t h_dst_idx = 0; h_dst_idx < dst_pad_h; h_dst_idx += blk_dst_pad_h) {
        ...
        for (int64_t w_dst_idx = 0; w_dst_idx < dst_pad_w; w_dst_idx += blk_dst_pad_w) {
            ...
            conv2d_conv_wg_s1_blocking_src_fp32(src, src_h, src_w, pad_channels, wgf, wgf, h_src_idx, w_src_idx,
                                                real_blk_src_pad_h, real_blk_src_pad_w, src_pad_blk);
            ...
            // src trans
            {
                ...
                for (int64_t i = 0; i < pad_channels; i += blk_channels) {
                    ...
                    conv2d_conv_wg_bxfxs1_src_blk_trans_fp32<wgb, wgf, src_trans_kernel_func>(
                        src_pad_blk_d, trans_mat_src, real_blk_src_pad_h, real_blk_src_pad_w, real_blk_channels,
                        real_blk_num_h_tile, real_blk_num_w_tile, src_trans_d);
                ...
                for (int64_t i = 0; i < pad_num_outs; i += blk_num_outs) {
                    ...
                    int64_t j = 0;
                    {
                        ...
                        for (int64_t k = 0; k < src_tile_size; k += 1) {
                            gemm_first_func(filter_d, src_trans_d, dst_trans_d, real_blk_num_outs, real_blk_num_tile,
                                            real_blk_channels);
                    ...
                    for (; j < pad_channels; j += blk_channels) {
                        ...
                        for (int64_t k = 0; k < src_tile_size; k += 1) {
                            gemm_func(filter_d, src_trans_d, dst_trans_d, real_blk_num_outs, real_blk_num_tile,
                                      real_blk_channels);
                    ...
                    conv2d_conv_wg_bxfxs1_dst_blk_trans_fp32<wgb, wgf, dst_trans_kernel_func>(
                        dst_trans, trans_mat_dst, bias + i, real_blk_dst_h, real_blk_dst_w, real_blk_dst_pad_h,
                        real_blk_dst_pad_w, real_blk_num_outs, real_blk_num_h_tile, real_blk_num_w_tile,
...
template <int64_t wgb, int64_t wgf, conv_wg_riscv_fp32_n4chw_src_trans_kernel_func_t src_trans_kernel_func,
          conv_wg_riscv_fp32_n4chw_dst_trans_kernel_func_t dst_trans_kernel_func>
void conv2d_conv_wg_bxfxs1_riscv_per_group_fp32(const float* src, const float* filter, const float* bias,
