...
#include "ppl/kernel/riscv/fp32/fc/vec128/kernel/fc_ndarray_kernel_fp32_vec128.h"
...
uint64_t fc_ndarray_fp32_vec128_executor::cal_temp_buffer_size()
{
    ...
    return fc_ndarray_common_cal_temp_buffer_size<float, atom_oc, atom_ic, flt_atom_oc>(
...
ppl::common::RetCode fc_ndarray_fp32_vec128_executor::prepare()
{
...
ppl::common::RetCode fc_ndarray_fp32_vec128_executor::execute()
{
    ...
    constexpr int64_t atom_ic = 4;
    constexpr int64_t flt_atom_oc = 16;
    fc_ndarray_common_blocking_execute<float, atom_oc, atom_ic, flt_atom_oc>(
        src_,
        cvt_filter_,
        cvt_bias_,
        dst_,
        ...
        tunning_param_,
        fc_ndarray_select_gemm_kernel_fp32_vec128<true>,
        fc_ndarray_select_gemm_kernel_fp32_vec128<false>
    );
    return common::RC_SUCCESS;
}

ppl::common::RetCode fc_ndarray_fp32_vec128_manager::gen_cvt_weights(const float* filter, const float* bias)
{
...

    {
        ...
        const int32_t flt_padded_oc = round_up(param_.num_output, 16);
        cvt_filter_size_        = padded_ic * flt_padded_oc * sizeof(float);
        cvt_filter_             = (float*)allocator_->Alloc(cvt_filter_size_);
        if (cvt_filter_ == nullptr) {
            return ppl::common::RC_OUT_OF_MEMORY;
        }
        fc_ndarray_common_cvt_flt_to_nxcx<float, 4, 16>(filter, cvt_filter_, param_.num_output, param_.channels);
    }
    return ppl::common::RC_SUCCESS;
}

fc_executor<float>* fc_ndarray_fp32_vec128_manager::gen_executor()
{
    return new fc_ndarray_fp32_vec128_executor(&param_, cvt_filter_, cvt_bias_);
}
