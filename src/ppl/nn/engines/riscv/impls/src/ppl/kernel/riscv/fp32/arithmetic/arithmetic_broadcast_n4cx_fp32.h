...
static inline void arithmetic_broadcast_lastdim_no_broadcast_n4cx_fp32(const float* src0, const float* src1, float* dst,
    ...
    if (!c0_broadcast && !c1_broadcast) {
        ...
        for (; i + C_BLK() < end * C_BLK(); i += C_BLK()) {
            ...
            float32xm1_t vfdata;
            vfdata = arithmetic_vector_kernel_fp32<op>(vlev_float32xm1(src0_, vl),
                                                       vlev_float32xm1(src1_, vl)); //  broadcast ?
            if (fuse_relu) {
                vsev_float32xm1(dst_, vfmaxvf_float32xm1(vfdata, (float)0.0f, vl), vl);
            } else {
                vsev_float32xm1(dst_, vfdata, vl);
...
template <arithmetic_op_type_t op, bool fuse_relu>
static inline void arithmetic_broadcast_lastdim_src0_broadcast_n4cx_fp32(const float* src0, const float* src1,
                                                                         float* dst,
...
template <arithmetic_op_type_t op, bool fuse_relu>
static inline void arithmetic_broadcast_lastdim_src1_broadcast_n4cx_fp32(const float* src0, const float* src1,
                                                                         float* dst,
...
static ppl::common::RetCode arithmetic_broadcast_recursive_n4cx_fp32(
    ...
    if (dim_idx == dim_count - 1) {
        if (src0_shape[dim_idx] == src1_shape[dim_idx]) {
            arithmetic_broadcast_lastdim_no_broadcast_n4cx_fp32<op, fuse_relu>(src0, src1, dst, start, end,
                                                                               c0_broadcast, c1_broadcast);
        } else if (src0_shape[dim_idx] == 1) {
            arithmetic_broadcast_lastdim_src0_broadcast_n4cx_fp32<op, fuse_relu>(src0, src1, dst, start, end,
                                                                                 c0_broadcast, c1_broadcast);
        } else if (src1_shape[dim_idx] == 1) {
            arithmetic_broadcast_lastdim_src1_broadcast_n4cx_fp32<op, fuse_relu>(src0, src1, dst, start, end,
                                                                                 c0_broadcast, c1_broadcast);
        }
    } else {
...
template <arithmetic_op_type_t op, bool fuse_relu>
static ppl::common::RetCode arithmetic_broadcast_n4cx_fp32(const float* src0, const float* src1, float* dst,
    ...
    const int64_t total_len = dst_shape->GetElementsIncludingPadding() / C_BLK();
    ...
    arithmetic_broadcast_recursive_n4cx_fp32<op, fuse_relu>(src0, src1, dst,
