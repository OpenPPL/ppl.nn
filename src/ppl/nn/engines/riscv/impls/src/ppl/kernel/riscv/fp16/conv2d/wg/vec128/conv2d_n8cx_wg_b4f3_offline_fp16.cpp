...
inline void wb_b4f3s1_cvt_filter_blk_kernel(const __fp16* filter,
...
uint64_t conv2d_n8cx_wg_b4f3_fp16_runtime_executor::cal_temp_buffer_size() {
    ...
    size_t temp_buffer_size = conv_wg_bxfxs1_get_temp_buffer_size_fp16<4, 3>(src_shape_->GetDim(2), // src_h,
...
ppl::common::RetCode conv2d_n8cx_wg_b4f3_fp16_offline_manager::pick_best_tunning_param(
    ...
    for (tunning_param_.oc_blk = 8; tunning_param_.oc_blk <= round_up(param_.num_output / param_.group, 8);
         tunning_param_.oc_blk *= 2) {
        for (tunning_param_.ic_blk = 8; tunning_param_.ic_blk <= round_up(param_.channels / param_.group, 8);
             tunning_param_.ic_blk *= 2) {
            for (tunning_param_.oh_blk = 16; tunning_param_.oh_blk <= dst_shape.GetDim(2);
                 tunning_param_.oh_blk += 16) {
                double inner_prev_time = DBL_MAX;
                for (tunning_param_.ow_blk = 16; tunning_param_.ow_blk <= dst_shape.GetDim(3);
                     tunning_param_.ow_blk += 16) {
...
ppl::common::RetCode conv2d_n8cx_wg_b4f3_fp16_offline_manager::gen_cvt_weights(const __fp16* filter,
    ...
    }
    {
        ...
        conv_wg_bxfxs1_cvt_filter_fp16<4, 3, wb_b4f3s1_cvt_filter_blk_kernel>(filter, // filter,
