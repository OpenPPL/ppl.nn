...
RetCode IfKernel::SetExecutionInfo(const shared_ptr<ir::GraphTopo>& then_topo, const RuntimeGraphInfo* then_info,
    ...
    auto status = then_branch_.Init(
        ...
        shared_ptr<const RuntimeAuxInfo>(then_aux_info, DummyDeleter<const RuntimeAuxInfo>));
    ...
    status = else_branch_.Init(else_topo,
                               shared_ptr<const RuntimeGraphInfo>(else_info, DummyDeleter<const RuntimeGraphInfo>),
                               shared_ptr<const RuntimeAuxInfo>(else_aux_info, DummyDeleter<const RuntimeAuxInfo>));
    if (status != RC_SUCCESS) {
