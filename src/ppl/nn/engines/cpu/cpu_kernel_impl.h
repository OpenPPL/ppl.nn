...
#ifndef _ST_HPC_PPL_NN_ENGINES_CPU_CPU_KERNEL_IMPL_H_
#define _ST_HPC_PPL_NN_ENGINES_CPU_CPU_KERNEL_IMPL_H_

#include "ppl/nn/runtime/kernel_impl.h"
#include "ppl/nn/engines/cpu/cpu_timing_guard.h"

namespace ppl { namespace nn { namespace cpu {

class CpuKernelImpl : public KernelImpl {
public:
    CpuKernelImpl(const ir::Node* node) : KernelImpl(node) {}

    ppl::common::RetCode Execute(KernelExecContext* ctx) override final {
#ifdef PPLNN_ENABLE_KERNEL_PROFILING
        CpuTimingGuard __timing_guard__(&begin_ts_, &end_ts_, ctx->IsProfilingEnabled());
#endif
