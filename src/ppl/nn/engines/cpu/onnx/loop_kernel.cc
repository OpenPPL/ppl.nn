...
#include "ppl/nn/engines/cpu/onnx/loop_kernel.h"
...
namespace ppl { namespace nn { namespace cpu { namespace onnx {
...
static RetCode UpdateSubgraphInputs(int64_t trip_count, RuntimeImpl* subgraph, GenericCpuDevice* tmp_cpu_device) {
...
static RetCode InitSubgraphInputs(const KernelExecContext& ctx, bool keep_going, GenericCpuDevice* tmp_cpu_device,
                                  RuntimeImpl* subgraph) {
    RetCode status;
...
RetCode LoopKernel::DoExecute(KernelExecContext* ctx) {
    ...
    GenericCpuDevice tmp_cpu_device;
