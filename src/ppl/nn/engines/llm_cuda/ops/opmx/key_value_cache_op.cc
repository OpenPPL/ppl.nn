...
#include "ppl/nn/engines/llm_cuda/kernels/opmx/key_value_cache_kernel.h"
#include "ppl/nn/oputils/opmx/reshape_key_value_cache.h"
...
RetCode KeyValueCacheOp::CommonInit() {
    ...
    infer_dims_func_ = [this](InputOutputInfo* info) -> RetCode {
        ...
        if (!start_pos->GetBufferPtr()) {
            nn::opmx::ReshapeKeyValueCache(info, param_.get(), &start_pos_val);
            return RC_NOT_FOUND;
        }
        ...
        return nn::opmx::ReshapeKeyValueCache(info, param_.get(), &start_pos_val);
    };
    return RC_SUCCESS;
}
