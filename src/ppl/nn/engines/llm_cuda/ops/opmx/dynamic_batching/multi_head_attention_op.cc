...
#include "ppl/nn/engines/llm_cuda/kernels/opmx/dynamic_batching/multi_head_attention_kernel.h"
...
RetCode DynamicBatchingMultiHeadAttentionOp::DoInit(const OptKernelOptions& options) {
    auto status = GenericLoadParam<ppl::nn::opmx::MultiHeadAttentionParam>(options, &param_);
...
ppl::common::RetCode DynamicBatchingMultiHeadAttentionOp::SerializeData(const ppl::nn::pmx::SerializationContext& ctx, utils::DataStream* ds) const {
    ...
    auto fb_param = opmx::CreateMultiHeadAttentionParam(builder, 
