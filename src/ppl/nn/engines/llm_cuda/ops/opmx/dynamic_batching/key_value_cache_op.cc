...
#include "ppl/nn/engines/llm_cuda/kernels/opmx/dynamic_batching/key_value_cache_kernel.h"
#include "ppl/nn/oputils/opmx/reshape_dynamic_batching_key_value_cache.h"
...
RetCode DynamicBatchingKeyValueCacheOp::CommonInit() {
    ...
    infer_dims_func_ = [this](InputOutputInfo* info) -> RetCode {
        ...
        if (!kvstarts->GetBufferPtr()) {
            ppl::nn::opmx::ReshapeDynamicBatchingKeyValueCache(info, param_.get(), kv_length);
            return RC_NOT_FOUND;
        }
        ...
        return ppl::nn::opmx::ReshapeDynamicBatchingKeyValueCache(info, param_.get(), kv_length);
    };
    return RC_SUCCESS;
}

RetCode DynamicBatchingKeyValueCacheOp::DoInit(const OptKernelOptions& options) {
    auto status = GenericLoadParam<ppl::nn::opmx::KeyValueCacheParam>(options, &param_);
    if (status != RC_SUCCESS) {
...
ppl::common::RetCode DynamicBatchingKeyValueCacheOp::SerializeData(const ppl::nn::pmx::SerializationContext& ctx, utils::DataStream* ds) const {
    ...
    auto fb_param = opmx::CreateKeyValueCacheParam(builder, 
...
ppl::common::RetCode DynamicBatchingKeyValueCacheOp::DeserializeData(const ppl::nn::pmx::DeserializationContext& ctx, const void* base, uint64_t size) {
    ...
    param_ = make_shared<ppl::nn::opmx::KeyValueCacheParam>();
