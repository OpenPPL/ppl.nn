...
#include "ppl/nn/engines/llm_cuda/kernels/opmx/parallel_embedding_kernel.h"
...
RetCode ParallelEmbeddingOp::CommonInit() {
    ...
    infer_dims_func_ = [this](InputOutputInfo* info) -> RetCode {
        return nn::opmx::ReshapeParallelEmbedding(info, param_.get(), nccl_param_->size);
    };
    return RC_SUCCESS;
}

RetCode ParallelEmbeddingOp::DoInit(const OptKernelOptions& options) {
    auto status = GenericLoadParam<ppl::nn::opmx::ParallelEmbeddingParam>(options, &param_);
...
ppl::common::RetCode ParallelEmbeddingOp::SerializeData(const ppl::nn::pmx::SerializationContext& ctx, utils::DataStream* ds) const {
    ...
    auto fb_param = opmx::CreateParallelEmbeddingParam(builder, 
        param_.get()->num_embeddings,
        param_.get()->embedding_dims,
        param_.get()->padding_idx,
        param_.get()->max_norm,
        param_.get()->norm_type);
    auto fb_op_param = opmx::CreateOpParam(builder, opmx::OpParamType_ParallelEmbeddingParam, fb_param.Union());
...
ppl::common::RetCode ParallelEmbeddingOp::DeserializeData(const ppl::nn::pmx::DeserializationContext& ctx, const void* base, uint64_t size) {
    ...
    param_ = make_shared<ppl::nn::opmx::ParallelEmbeddingParam>();
