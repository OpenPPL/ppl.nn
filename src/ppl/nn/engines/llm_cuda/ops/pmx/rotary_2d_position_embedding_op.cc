...
#include "ppl/nn/engines/llm_cuda/kernels/pmx/rotary_2d_position_embedding_kernel.h"
...
RetCode Rotary2DPositionEmbeddingOp::CommonInit() {
    infer_type_and_format_func_ = GenericInferTypeAndFormat;
    infer_dims_func_ = GenericInferDims;
    return RC_SUCCESS;
}

RetCode Rotary2DPositionEmbeddingOp::DoInit(const OptKernelOptions& options) {
...
KernelImpl* Rotary2DPositionEmbeddingOp::CreateKernelImpl() const {
    return CreateKernelImplWithParam<Rotary2DPositionEmbeddingKernel>(param_.get());
}

#ifdef PPLNN_ENABLE_PMX_MODEL
ppl::common::RetCode Rotary2DPositionEmbeddingOp::SerializeData(const ppl::nn::pmx::SerializationContext& ctx, utils::DataStream* ds) const {
...
ppl::common::RetCode Rotary2DPositionEmbeddingOp::DeserializeData(const ppl::nn::pmx::DeserializationContext& ctx, const void* base, uint64_t size) {
