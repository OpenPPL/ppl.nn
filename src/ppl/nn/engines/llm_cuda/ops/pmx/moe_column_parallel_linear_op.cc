...
#include "ppl/nn/engines/llm_cuda/kernels/pmx/moe_column_parallel_linear_kernel.h"
...
RetCode MoeColumnParallelLinearOp::CommonInit() {
    ...
    infer_dims_func_ = [this](InputOutputInfo* info) -> RetCode {
        return nn::pmx::ReshapeMoeColumnParallelLinear(info, param_.get(), nccl_param_->size);
    };
    return RC_SUCCESS;
}

RetCode MoeColumnParallelLinearOp::DoInit(const OptKernelOptions& options) {
    auto status = GenericLoadParam<MoeColumnParallelLinearParam>(options, &param_);
