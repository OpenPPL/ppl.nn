...
ppl::common::RetCode KeyValueCacheKernel::DoExecute(KernelExecContext* ctx) {
    ...
    PPLNN_LLM_CUDA_REQUIRED_INPUT(cache, 3);
    PPLNN_LLM_CUDA_OPTIONAL_INPUT(scale, 4);
    ...
    int64_t start_pos_val;
    auto status = start_pos->CopyToHost(&start_pos_val);
    if (status != ppl::common::RC_SUCCESS) {
        LOG(ERROR) << "start_pos->CopyToHost() FAILED: " << ppl::common::GetRetCodeStr(status);
        return status;
    }
