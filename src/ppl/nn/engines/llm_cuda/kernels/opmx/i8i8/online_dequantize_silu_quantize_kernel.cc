...
#include "online_dequantize_silu_quantize_kernel.h"
...
ppl::common::RetCode I8I8OnlineDequantizeSiLUQuantizeKernel::DoExecute(KernelExecContext* ctx) {
    ...
    PPLNN_LLM_CUDA_OPTIONAL_INPUT(gate, 3);
    PPLNN_LLM_CUDA_OPTIONAL_INPUT(gate_scale_outer, 4);
    PPLNN_LLM_CUDA_OPTIONAL_INPUT(gate_scale_inner, 5);
    ...
    if (gate_scale_outer) {
        PPLNN_LLM_CUDA_DEBUG_TRACE("Input [gate_scale_outer]:\n");
        PPLNN_LLM_CUDA_TENSOR_PRINT_DEBUG_MSG(gate_scale_outer);
    }
    if (gate_scale_inner) {
        PPLNN_LLM_CUDA_DEBUG_TRACE("Input [gate_scale_inner]:\n");
        PPLNN_LLM_CUDA_TENSOR_PRINT_DEBUG_MSG(gate_scale_inner);
    }
    ...
    if (gate) {
        if (gate_scale_outer == nullptr) {
            LOG(ERROR) << "scale_outer of gate not found";
            return ppl::common::RC_NOT_FOUND;
        }

        if (gate_scale_inner == nullptr) {
            LOG(ERROR) << "scale_inner of gate not found";
            return ppl::common::RC_NOT_FOUND;
        }

        if (gate_scale_outer->GetShape()->GetDataType() != gate_scale_inner->GetShape()->GetDataType()) {
            LOG(ERROR) << "datatype of scale_outer must be equal to datatype of scale_inner: "
                << ppl::common::GetDataTypeStr(gate_scale_outer->GetShape()->GetDataType()) << " vs. "
                << ppl::common::GetDataTypeStr(gate_scale_inner->GetShape()->GetDataType());
            return ppl::common::RC_INVALID_VALUE;
        }

        if (gate_scale_outer->GetShape()->GetDataType() != ppl::common::DATATYPE_FLOAT16) {
            LOG(ERROR) << "currently only support dequantize to fp16";
            return ppl::common::RC_UNSUPPORTED;
        }

        if (gate->GetShape()->GetDataType() != ppl::common::DATATYPE_INT32) {
            LOG(ERROR) << "currently only support dequantize int32 data";
            return ppl::common::RC_UNSUPPORTED;
        }
    }
    ...
    uint64_t dequant_buffer_size = input->GetShape()->CalcElementsIncludingPadding() * sizeof(int16_t);
    ...
    return ppl::kernel::llm::cuda::pmx::i8i8::minmax_requantize_silu_fp16(
        GetStream(),
        input->GetBufferPtr(),
        input_scale_outer->GetBufferPtr(),
        input_scale_inner->GetBufferPtr(),
        gate ? gate->GetBufferPtr() : nullptr,
        gate ? gate_scale_outer->GetBufferPtr() : nullptr,
        gate ? gate_scale_inner->GetBufferPtr() : nullptr,
        batch,
        quant_dim,
        tensor_layout,
