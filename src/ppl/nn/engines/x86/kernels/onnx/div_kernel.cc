...
ppl::common::RetCode DivKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (data_type == common::DATATYPE_FLOAT32) {
        if (MayUseISA(ppl::common::ISA_X86_AVX)) {
            return kernel::x86::div_fp32_avx(&A->GetShape(), &B->GetShape(), &C->GetShape(),
                                             A->GetBufferPtr<const float>(), B->GetBufferPtr<const float>(), fuse_relu_,
                                             C->GetBufferPtr<float>());
        } else if (MayUseISA(ppl::common::ISA_X86_SSE)) {
            return kernel::x86::div_fp32_sse(&A->GetShape(), &B->GetShape(), &C->GetShape(),
                                             A->GetBufferPtr<const float>(), B->GetBufferPtr<const float>(), fuse_relu_,
                                             C->GetBufferPtr<float>());
        } else {
        ...
        return kernel::x86::div_int64(&A->GetShape(), &B->GetShape(), &C->GetShape(), A->GetBufferPtr<const int64_t>(),
