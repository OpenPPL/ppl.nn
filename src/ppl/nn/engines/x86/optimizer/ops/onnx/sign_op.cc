...
RetCode SignOp::DoInit(const OptKernelOptions& options) {
    infer_dims_func_ = GenericInferDims;
    infer_type_func_ = GenericInferType;
    return RC_SUCCESS;
}

RetCode SignOp::SelectFormat(const InputOutputInfo& info, vector<dataformat_t>* selected_input_formats,
...
KernelImpl* SignOp::CreateKernelImpl() const {
    return CreateKernelImplWithoutParam<SignKernel>();
}
