...
#define IC_COMPUTE_STEP(IC) do {\
    if (oc_len > 0 * CH_DT_BLK()) zmm24 = _mm512_loadu_ps(ic_flt_o16 + 0 * flt_ocb_stride + (IC) * CH_DT_BLK());\
    if (oc_len > 1 * CH_DT_BLK()) zmm25 = _mm512_loadu_ps(ic_flt_o16 + 1 * flt_ocb_stride + (IC) * CH_DT_BLK());\
    if (oc_len > 2 * CH_DT_BLK()) zmm26 = _mm512_loadu_ps(ic_flt_o48 + 0 * flt_ocb_stride + (IC) * CH_DT_BLK());\
    if (oc_len > 3 * CH_DT_BLK()) zmm27 = _mm512_loadu_ps(ic_flt_o48 + 1 * flt_ocb_stride + (IC) * CH_DT_BLK());\
    if (w_len > 0) {\
...
#define IC_PREFETCH_STEP(IC)  do {\
    if (oc_len > 3 * CH_DT_BLK() && w_len > 3) {\
        ...
        if (oc_len > 3 * CH_DT_BLK()) _mm_prefetch((const char*)ic_flt_o48 + 1 * flt_ocb_stride + (IC) * CH_DT_BLK() + CH_DT_BLK() * CH_DT_BLK(), _MM_HINT_T0);\
    }\
} while (0)
