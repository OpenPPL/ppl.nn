                    ...
                    if (mode == 0) { // avg pooling
                        ...
                        for (int64_t i = 0; i < roi_bin_grid_h * roi_bin_grid_w; i++) {
                            ...
                            for (int64_t cc = 0; cc < c_blk; cc++) {
                                output_val[cc] += pc.w1 * p_src[pc.pos1 * c_blk + cc] + pc.w2 * p_src[pc.pos2 * c_blk + cc] +
                                                  pc.w3 * p_src[pc.pos3 * c_blk + cc] + pc.w4 * p_src[pc.pos4 * c_blk + cc];
                            }
                            pre_calc_index++;
                        }
                        for (int64_t cc = 0; cc < c_blk; cc++) {
                            dst[index + cc] = output_val[cc] * r_count;
                        }
                        ...
                        for (int64_t i = 0; i < roi_bin_grid_h * roi_bin_grid_w; i++) {
                            ...
                            for (int64_t cc = 0; cc < c_blk; cc++) {
                                ...
                                output_val[cc] = max(output_val[cc], val);
                            }
                            pre_calc_index++;
                        }
                        for (int64_t cc = 0; cc < c_blk; cc++) {
                            dst[index + cc] = output_val[cc];
                        }
                    }
                }
            }
