    ...
    float *output)
{
    ...
    for (int64_t n = 0; n < n_rois; n++) {
        ...
        const __m256 v_rcount  = _mm256_set1_ps(r_count);
                ...
                for (int64_t pw = 0; pw < pooled_width; pw++) {
                    ...
                    if (pool_mode == 1) { // avg pooling
                        ...
                        for (int64_t i = 0; i < roi_bin_grid_h * roi_bin_grid_w; i++) {
                            ...
                            const __m256 v_src1_lo = _mm256_loadu_ps(offset_input + pc.pos1 * c_blk);
                            const __m256 v_src2_lo = _mm256_loadu_ps(offset_input + pc.pos2 * c_blk);
                            const __m256 v_src3_lo = _mm256_loadu_ps(offset_input + pc.pos3 * c_blk);
                            const __m256 v_src4_lo = _mm256_loadu_ps(offset_input + pc.pos4 * c_blk);

                            const __m256 v_src1_hi = _mm256_loadu_ps(offset_input + pc.pos1 * c_blk + 8);
                            const __m256 v_src2_hi = _mm256_loadu_ps(offset_input + pc.pos2 * c_blk + 8);
                            const __m256 v_src3_hi = _mm256_loadu_ps(offset_input + pc.pos3 * c_blk + 8);
                            const __m256 v_src4_hi = _mm256_loadu_ps(offset_input + pc.pos4 * c_blk + 8);
                        ...
                        _mm256_storeu_ps(output + index, v_dst_lo * v_rcount);
                        _mm256_storeu_ps(output + index + 8, v_dst_hi * v_rcount);
                        ...
                        for (int64_t i = 0; i < roi_bin_grid_h * roi_bin_grid_w; i++) {
                            ...
                            v_dst_lo = _mm256_max_ps(v_dst_lo, v_w1 * v_src1_lo + v_w2 * v_src2_lo + v_w3 * v_src3_lo + v_w4 * v_src4_lo);
                            v_dst_hi = _mm256_max_ps(v_dst_hi, v_w1 * v_src1_hi + v_w2 * v_src2_hi + v_w3 * v_src3_hi + v_w4 * v_src4_hi);
                            pre_calc_index++;
                        }
                        _mm256_storeu_ps(output + index, v_dst_lo);
                        _mm256_storeu_ps(output + index + 8, v_dst_hi);
                    }
                } // for pw
            } // for ph
        } // for c
