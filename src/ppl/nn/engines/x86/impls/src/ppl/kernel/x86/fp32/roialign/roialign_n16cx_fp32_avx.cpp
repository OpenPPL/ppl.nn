    ...
    float *dst)
{
    ...
    for (int i = 0; i < num_rois; i++) {
        ...
        const __m256 v_zero    = _mm256_set1_ps(0);
        const __m256 v_flt_min = _mm256_set1_ps(-FLT_MAX);
        const __m256 v_rcount  = _mm256_set1_ps(1.0f / count);
            ...
            for (int64_t ph = 0; ph < pooled_height; ph++) {
                for (int64_t pw = 0; pw < pooled_width; pw++) {
                    ...
                    if (mode == 0) { // avg pooling
                        __m256 v_dst_lo = v_zero;
                        __m256 v_dst_hi = v_zero;
                        for (int64_t i = 0; i < roi_bin_grid_h * roi_bin_grid_w; i++) {
                            pre_calc_info pc  = pre_calc[pre_calc_index];
                            const __m256 v_w1 = _mm256_set1_ps(pc.w1);
                            const __m256 v_w2 = _mm256_set1_ps(pc.w2);
                            const __m256 v_w3 = _mm256_set1_ps(pc.w3);
                            const __m256 v_w4 = _mm256_set1_ps(pc.w4);

                            const __m256 v_src1_lo = _mm256_loadu_ps(p_src + pc.pos1 * c_blk);
                            const __m256 v_src2_lo = _mm256_loadu_ps(p_src + pc.pos2 * c_blk);
                            const __m256 v_src3_lo = _mm256_loadu_ps(p_src + pc.pos3 * c_blk);
                            const __m256 v_src4_lo = _mm256_loadu_ps(p_src + pc.pos4 * c_blk);

                            const __m256 v_src1_hi = _mm256_loadu_ps(p_src + pc.pos1 * c_blk + 8);
                            const __m256 v_src2_hi = _mm256_loadu_ps(p_src + pc.pos2 * c_blk + 8);
                            const __m256 v_src3_hi = _mm256_loadu_ps(p_src + pc.pos3 * c_blk + 8);
                            const __m256 v_src4_hi = _mm256_loadu_ps(p_src + pc.pos4 * c_blk + 8);

                            v_dst_lo = v_dst_lo + (v_w1 * v_src1_lo + v_w2 * v_src2_lo + v_w3 * v_src3_lo + v_w4 * v_src4_lo);
                            v_dst_hi = v_dst_hi + (v_w1 * v_src1_hi + v_w2 * v_src2_hi + v_w3 * v_src3_hi + v_w4 * v_src4_hi);
                            pre_calc_index++;
                        }
                        _mm256_storeu_ps(dst + index, v_dst_lo * v_rcount);
                        _mm256_storeu_ps(dst + index + 8, v_dst_hi * v_rcount);
                    } else if (mode == 1) { // max pooling
                        __m256 v_dst_lo = v_flt_min;
                        __m256 v_dst_hi = v_flt_min;
                        for (int64_t i = 0; i < roi_bin_grid_h * roi_bin_grid_w; i++) {
                            pre_calc_info pc  = pre_calc[pre_calc_index];
                            const __m256 v_w1 = _mm256_set1_ps(pc.w1);
                            const __m256 v_w2 = _mm256_set1_ps(pc.w2);
                            ...
                            v_dst_lo = _mm256_max_ps(v_dst_lo, _mm256_max_ps(_mm256_max_ps(v_w1 * v_src1_lo, v_w2 * v_src2_lo), _mm256_max_ps(v_w3 * v_src3_lo, v_w4 * v_src4_lo)));
                            v_dst_hi = _mm256_max_ps(v_dst_hi, _mm256_max_ps(_mm256_max_ps(v_w1 * v_src1_hi, v_w2 * v_src2_hi), _mm256_max_ps(v_w3 * v_src3_hi, v_w4 * v_src4_hi)));
                            pre_calc_index++;
                        }
                        _mm256_storeu_ps(dst + index, v_dst_lo);
                        _mm256_storeu_ps(dst + index + 8, v_dst_hi);
                    }
                }
            }
        }
