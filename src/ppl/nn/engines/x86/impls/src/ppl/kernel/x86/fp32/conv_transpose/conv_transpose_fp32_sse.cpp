    ...
    const int32_t pad_w)
{
    ...
    param.ldb      = param.N;
    param.ldy   = param.N;
    param.isa_flag = common::ISA_X86_SSE;
    param.trans_A   = 1;
    param.trans_B   = 0;
    ...
    for (int64_t b = 0; b < batch; ++b) {
        ...
        if (nullptr != bias) {
            ...
            for (int64_t oc = 0; oc < num_output; ++oc) {
                __m128 vbias = _mm_set1_ps(bias[oc]);
                for (int64_t hw = 0; hw < round(dst_h * dst_w, simd_w); hw += simd_w) {
                    __m128 vdst = _mm_loadu_ps(dst_d + oc * dst_h * dst_w + hw);
                    vdst        = _mm_add_ps(vdst, vbias);
                    _mm_storeu_ps(dst_d + oc * dst_h * dst_w + hw, vdst);
                }
