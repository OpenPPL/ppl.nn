    ...
    float *Y_c)
{
    ...
    if (isa & ppl::common::ISA_X86_AVX512) {
        return kernel::x86::lstm_fp32_avx512(X_shape, X, W, R, P, bias, sequence_lens, initial_h, initial_c, direction, hidden_size, packed_W, packed_R, temp_buffer, Y, Y_h, Y_c);
    }
#endif
    if (isa & ppl::common::ISA_X86_FMA) {
        return kernel::x86::lstm_fp32_fma(X_shape, X, W, R, P, bias, sequence_lens, initial_h, initial_c, direction, hidden_size, packed_W, packed_R, temp_buffer, Y, Y_h, Y_c);
    }
    return kernel::x86::lstm_fp32_sse(X_shape, X, W, R, P, bias, sequence_lens, initial_h, initial_c, direction, hidden_size, packed_W, packed_R, temp_buffer, Y, Y_h, Y_c);
}
