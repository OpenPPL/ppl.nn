...
bool conv2d_n16cx_winograd_b2f5s2_fp32_avx512_executor::init_profiler()
{
...
void conv2d_n16cx_winograd_b2f5s2_fp32_avx512_executor::clear_profiler()
{
#ifdef PPL_X86_KERNEL_TIMING
    profiler_.clear();
#endif
...
std::string conv2d_n16cx_winograd_b2f5s2_fp32_avx512_executor::export_profiler()
{
...
void conv2d_n16cx_winograd_b2f5s2_fp32_avx512_executor::init_preproc_param()
{
...
uint64_t conv2d_n16cx_winograd_b2f5s2_fp32_avx512_executor::cal_temp_buffer_size()
{
...
ppl::common::RetCode conv2d_n16cx_winograd_b2f5s2_fp32_avx512_executor::prepare()
{
...
static inline void winograd_b2f5s2_preprocess_fp32_avx512(
    ...
    float *src_trans)
{
    ...
    if (ih >= 0 && ih + TILE_IN_H() <= src_h && iw >= 0 && iw + TILE_IN_W() <= src_w) {
        ...
        int64_t tl_pad       = max<int64_t>(-iw, 0);
    ...
    __m512 zmm0, zmm1, zmm2, zmm3, zmm4, zmm5, zmm6, zmm7, zmm8;

    for (int64_t tw = 0; tw < TILE_IN_W(); ++tw) {
        const float *l_tile = tile_src + tw * CH_DT_BLK();
        float *l_temp = matmul_buffer + tw * CH_DT_BLK();

        zmm0 = _mm512_loadu_ps(l_tile + 0 * tile_src_h_stride);
        zmm1 = _mm512_loadu_ps(l_tile + 1 * tile_src_h_stride);
        zmm2 = _mm512_loadu_ps(l_tile + 2 * tile_src_h_stride);
        zmm3 = _mm512_loadu_ps(l_tile + 3 * tile_src_h_stride);
        zmm4 = _mm512_loadu_ps(l_tile + 4 * tile_src_h_stride);
        zmm5 = _mm512_loadu_ps(l_tile + 5 * tile_src_h_stride);
        zmm6 = _mm512_loadu_ps(l_tile + 6 * tile_src_h_stride);

        zmm7 = zmm0 - zmm4;
        zmm8 = zmm1 - zmm5;
        _mm512_storeu_ps(l_temp + 0 * tile_h_stride, zmm7);
        _mm512_storeu_ps(l_temp + 1 * tile_h_stride, zmm8);
        zmm7 = zmm2 + zmm4;
        zmm8 = zmm3 + zmm5;
        _mm512_storeu_ps(l_temp + 2 * tile_h_stride, zmm7);
        _mm512_storeu_ps(l_temp + 3 * tile_h_stride, zmm8);
        zmm7 = zmm4 - zmm2;
        zmm8 = zmm5 - zmm3;
        _mm512_storeu_ps(l_temp + 4 * tile_h_stride, zmm7);
        _mm512_storeu_ps(l_temp + 5 * tile_h_stride, zmm8); 
        zmm7 = zmm6 - zmm2;
        _mm512_storeu_ps(l_temp + 6 * tile_h_stride, zmm7);
    }

    for(int64_t th = 0; th < TILE_IN_H(); ++th) {
        const float *l_temp = matmul_buffer + th * tile_h_stride;
        float *l_dst = src_trans + th * TILE_IN_W() * src_trans_ti_stride;

        zmm0 = _mm512_loadu_ps(l_temp + 0 * CH_DT_BLK());
        zmm1 = _mm512_loadu_ps(l_temp + 1 * CH_DT_BLK());
        zmm2 = _mm512_loadu_ps(l_temp + 2 * CH_DT_BLK());
        zmm3 = _mm512_loadu_ps(l_temp + 3 * CH_DT_BLK());
        zmm4 = _mm512_loadu_ps(l_temp + 4 * CH_DT_BLK());
        zmm5 = _mm512_loadu_ps(l_temp + 5 * CH_DT_BLK());
        zmm6 = _mm512_loadu_ps(l_temp + 6 * CH_DT_BLK());

        zmm7 = zmm0 - zmm4;
        zmm8 = zmm1 - zmm5;
        _mm512_storeu_ps(l_dst + 0 * src_trans_ti_stride, zmm7);
        _mm512_storeu_ps(l_dst + 1 * src_trans_ti_stride, zmm8);

        zmm7 = zmm2 + zmm4;
        zmm8 = zmm3 + zmm5;
        _mm512_storeu_ps(l_dst + 2 * src_trans_ti_stride, zmm7);
        _mm512_storeu_ps(l_dst + 3 * src_trans_ti_stride, zmm8);

        zmm7 = zmm4 - zmm2;
        zmm8 = zmm5 - zmm3;
        _mm512_storeu_ps(l_dst + 4 * src_trans_ti_stride, zmm7);
        _mm512_storeu_ps(l_dst + 5 * src_trans_ti_stride, zmm8);

        zmm7 = zmm6 - zmm2;
        _mm512_storeu_ps(l_dst + 6 * src_trans_ti_stride, zmm7);
    }
}

static inline void winograd_b2f5s2_dst_trans_fp32_avx512(
    ...
    float *dst)
{
    ...
    __m512 zmm7, zmm8, zmm9, zmm10, zmm11, zmm12, zmm13, zmm14;
    __m512 zmm15, zmm16, zmm17, zmm18, zmm19, zmm20, zmm21, zmm22;
    __m512 zmm23, zmm24, zmm25, zmm26, zmm27, zmm28, zmm29, zmm30;

    zmm0 = _mm512_loadu_ps(dst_trans + 0 * TILE_IN_W() * dst_trans_ti_stride + 0 * dst_trans_ti_stride);
    zmm1 = _mm512_loadu_ps(dst_trans + 1 * TILE_IN_W() * dst_trans_ti_stride + 0 * dst_trans_ti_stride);
    zmm2 = _mm512_loadu_ps(dst_trans + 2 * TILE_IN_W() * dst_trans_ti_stride + 0 * dst_trans_ti_stride);
    zmm3 = _mm512_loadu_ps(dst_trans + 3 * TILE_IN_W() * dst_trans_ti_stride + 0 * dst_trans_ti_stride);
    zmm4 = _mm512_loadu_ps(dst_trans + 4 * TILE_IN_W() * dst_trans_ti_stride + 0 * dst_trans_ti_stride);
    zmm5 = _mm512_loadu_ps(dst_trans + 5 * TILE_IN_W() * dst_trans_ti_stride + 0 * dst_trans_ti_stride);
    zmm6 = _mm512_loadu_ps(dst_trans + 6 * TILE_IN_W() * dst_trans_ti_stride + 0 * dst_trans_ti_stride);
    ...
    zmm0 = _mm512_loadu_ps(dst_trans + 0 * TILE_IN_W() * dst_trans_ti_stride + 1 * dst_trans_ti_stride);
    zmm1 = _mm512_loadu_ps(dst_trans + 1 * TILE_IN_W() * dst_trans_ti_stride + 1 * dst_trans_ti_stride);
    zmm2 = _mm512_loadu_ps(dst_trans + 2 * TILE_IN_W() * dst_trans_ti_stride + 1 * dst_trans_ti_stride);
    zmm3 = _mm512_loadu_ps(dst_trans + 3 * TILE_IN_W() * dst_trans_ti_stride + 1 * dst_trans_ti_stride);
    zmm4 = _mm512_loadu_ps(dst_trans + 4 * TILE_IN_W() * dst_trans_ti_stride + 1 * dst_trans_ti_stride);
    zmm5 = _mm512_loadu_ps(dst_trans + 5 * TILE_IN_W() * dst_trans_ti_stride + 1 * dst_trans_ti_stride);
    zmm6 = _mm512_loadu_ps(dst_trans + 6 * TILE_IN_W() * dst_trans_ti_stride + 1 * dst_trans_ti_stride);

    zmm11 = zmm0 + zmm2 + zmm4;
    zmm12 = zmm1 + zmm3 + zmm5;
    zmm13 = zmm2 - zmm4 + zmm6;
    zmm14 = zmm3 - zmm5;

    zmm0 = _mm512_loadu_ps(dst_trans + 0 * TILE_IN_W() * dst_trans_ti_stride + 2 * dst_trans_ti_stride);
    zmm1 = _mm512_loadu_ps(dst_trans + 1 * TILE_IN_W() * dst_trans_ti_stride + 2 * dst_trans_ti_stride);
    zmm2 = _mm512_loadu_ps(dst_trans + 2 * TILE_IN_W() * dst_trans_ti_stride + 2 * dst_trans_ti_stride);
    zmm3 = _mm512_loadu_ps(dst_trans + 3 * TILE_IN_W() * dst_trans_ti_stride + 2 * dst_trans_ti_stride);
    zmm4 = _mm512_loadu_ps(dst_trans + 4 * TILE_IN_W() * dst_trans_ti_stride + 2 * dst_trans_ti_stride);
    zmm5 = _mm512_loadu_ps(dst_trans + 5 * TILE_IN_W() * dst_trans_ti_stride + 2 * dst_trans_ti_stride);
    zmm6 = _mm512_loadu_ps(dst_trans + 6 * TILE_IN_W() * dst_trans_ti_stride + 2 * dst_trans_ti_stride);

    zmm15 = zmm0 + zmm2 + zmm4;
    zmm16 = zmm1 + zmm3 + zmm5;
    zmm17 = zmm2 - zmm4 + zmm6;
    zmm18 = zmm3 - zmm5;

    zmm0 = _mm512_loadu_ps(dst_trans + 0 * TILE_IN_W() * dst_trans_ti_stride + 3 * dst_trans_ti_stride);
    zmm1 = _mm512_loadu_ps(dst_trans + 1 * TILE_IN_W() * dst_trans_ti_stride + 3 * dst_trans_ti_stride);
    zmm2 = _mm512_loadu_ps(dst_trans + 2 * TILE_IN_W() * dst_trans_ti_stride + 3 * dst_trans_ti_stride);
    zmm3 = _mm512_loadu_ps(dst_trans + 3 * TILE_IN_W() * dst_trans_ti_stride + 3 * dst_trans_ti_stride);
    zmm4 = _mm512_loadu_ps(dst_trans + 4 * TILE_IN_W() * dst_trans_ti_stride + 3 * dst_trans_ti_stride);
    zmm5 = _mm512_loadu_ps(dst_trans + 5 * TILE_IN_W() * dst_trans_ti_stride + 3 * dst_trans_ti_stride);
    zmm6 = _mm512_loadu_ps(dst_trans + 6 * TILE_IN_W() * dst_trans_ti_stride + 3 * dst_trans_ti_stride);

    zmm19 = zmm0 + zmm2 + zmm4;
    zmm20 = zmm1 + zmm3 + zmm5;
    zmm21 = zmm2 - zmm4 + zmm6;
    zmm22 = zmm3 - zmm5;

    zmm0 = _mm512_loadu_ps(dst_trans + 0 * TILE_IN_W() * dst_trans_ti_stride + 4 * dst_trans_ti_stride);
    zmm1 = _mm512_loadu_ps(dst_trans + 1 * TILE_IN_W() * dst_trans_ti_stride + 4 * dst_trans_ti_stride);
    zmm2 = _mm512_loadu_ps(dst_trans + 2 * TILE_IN_W() * dst_trans_ti_stride + 4 * dst_trans_ti_stride);
    zmm3 = _mm512_loadu_ps(dst_trans + 3 * TILE_IN_W() * dst_trans_ti_stride + 4 * dst_trans_ti_stride);
    zmm4 = _mm512_loadu_ps(dst_trans + 4 * TILE_IN_W() * dst_trans_ti_stride + 4 * dst_trans_ti_stride);
    zmm5 = _mm512_loadu_ps(dst_trans + 5 * TILE_IN_W() * dst_trans_ti_stride + 4 * dst_trans_ti_stride);
    zmm6 = _mm512_loadu_ps(dst_trans + 6 * TILE_IN_W() * dst_trans_ti_stride + 4 * dst_trans_ti_stride);

    zmm23 = zmm0 + zmm2 + zmm4;
    zmm24 = zmm1 + zmm3 + zmm5;
    zmm25 = zmm2 - zmm4 + zmm6;
    zmm26 = zmm3 - zmm5;

    zmm0 = _mm512_loadu_ps(dst_trans + 0 * TILE_IN_W() * dst_trans_ti_stride + 5 * dst_trans_ti_stride);
    zmm1 = _mm512_loadu_ps(dst_trans + 1 * TILE_IN_W() * dst_trans_ti_stride + 5 * dst_trans_ti_stride);
    zmm2 = _mm512_loadu_ps(dst_trans + 2 * TILE_IN_W() * dst_trans_ti_stride + 5 * dst_trans_ti_stride);
    zmm3 = _mm512_loadu_ps(dst_trans + 3 * TILE_IN_W() * dst_trans_ti_stride + 5 * dst_trans_ti_stride);
    zmm4 = _mm512_loadu_ps(dst_trans + 4 * TILE_IN_W() * dst_trans_ti_stride + 5 * dst_trans_ti_stride);
    zmm5 = _mm512_loadu_ps(dst_trans + 5 * TILE_IN_W() * dst_trans_ti_stride + 5 * dst_trans_ti_stride);
    zmm6 = _mm512_loadu_ps(dst_trans + 6 * TILE_IN_W() * dst_trans_ti_stride + 5 * dst_trans_ti_stride);

    zmm27 = zmm0 + zmm2 + zmm4;
    zmm28 = zmm1 + zmm3 + zmm5;
    zmm29 = zmm2 - zmm4 + zmm6;
    zmm30 = zmm3 - zmm5;

    zmm0 = _mm512_loadu_ps(dst_trans + 0 * TILE_IN_W() * dst_trans_ti_stride + 6 * dst_trans_ti_stride);
    zmm1 = _mm512_loadu_ps(dst_trans + 1 * TILE_IN_W() * dst_trans_ti_stride + 6 * dst_trans_ti_stride);
    zmm2 = _mm512_loadu_ps(dst_trans + 2 * TILE_IN_W() * dst_trans_ti_stride + 6 * dst_trans_ti_stride);
    zmm3 = _mm512_loadu_ps(dst_trans + 3 * TILE_IN_W() * dst_trans_ti_stride + 6 * dst_trans_ti_stride);
    zmm4 = _mm512_loadu_ps(dst_trans + 4 * TILE_IN_W() * dst_trans_ti_stride + 6 * dst_trans_ti_stride);
    zmm5 = _mm512_loadu_ps(dst_trans + 5 * TILE_IN_W() * dst_trans_ti_stride + 6 * dst_trans_ti_stride);
    zmm6 = _mm512_loadu_ps(dst_trans + 6 * TILE_IN_W() * dst_trans_ti_stride + 6 * dst_trans_ti_stride);
    ...
    zmm7 = zmm7 + zmm15 + zmm23;
    zmm11 = zmm11 + zmm19 + zmm27;
    zmm15 = zmm15 - zmm23 + zmm0;
    zmm19 -= zmm27;

    zmm8 = zmm8 + zmm16 + zmm24;
    zmm12 = zmm12 + zmm20 + zmm28;
    zmm16 = zmm16 - zmm24 + zmm1;
    zmm20 -= zmm28;

    zmm9 = zmm9 + zmm17 + zmm25;
    zmm13 = zmm13 + zmm21 + zmm29;
    zmm17 = zmm17 - zmm25 + zmm2;
    zmm21 -= zmm29;

    zmm10 = zmm10 + zmm18 + zmm26;
    zmm14 = zmm14 + zmm22 + zmm30;
    zmm18 = zmm18 - zmm26 + zmm3;
    zmm22 -= zmm30;

    // add four part
    zmm5 = zmm7 + zmm8 + zmm11 + zmm12;
    zmm6 = zmm9 + zmm10 + zmm13 + zmm14;
    zmm7 = zmm15 + zmm16 + zmm19 + zmm20;
    zmm8 = zmm17 + zmm18 + zmm21 + zmm22;

    zmm4 = _mm512_loadu_ps(bias);
    ...
    if (fuse_flag & conv_fuse_flag::SUM) {
        zmm4 = _mm512_loadu_ps(sum_src + 0 * sum_src_h_stride + 0 * CH_DT_BLK());
        zmm5 += zmm4;
        zmm4 = _mm512_loadu_ps(sum_src + 0 * sum_src_h_stride + 1 * CH_DT_BLK());
        zmm6 += zmm4;
        zmm4 = _mm512_loadu_ps(sum_src + 1 * sum_src_h_stride + 0 * CH_DT_BLK());
        zmm7 += zmm4;
        zmm4 = _mm512_loadu_ps(sum_src + 1 * sum_src_h_stride + 1 * CH_DT_BLK());
        zmm8 += zmm4;
    }

    if (fuse_flag & (conv_fuse_flag::RELU | conv_fuse_flag::RELU6)) {
        zmm4 = _mm512_setzero_ps();
        zmm5  = _mm512_max_ps(zmm5, zmm4);
        zmm6  = _mm512_max_ps(zmm6, zmm4);
        zmm7  = _mm512_max_ps(zmm7, zmm4);
        zmm8  = _mm512_max_ps(zmm8, zmm4);
    }

    if (fuse_flag & conv_fuse_flag::RELU6) {
        zmm4 = _mm512_set1_ps(6.0f);
        zmm5  = _mm512_min_ps(zmm5, zmm4);
        zmm6  = _mm512_min_ps(zmm6, zmm4);
        zmm7  = _mm512_min_ps(zmm7, zmm4);
        zmm8  = _mm512_min_ps(zmm8, zmm4);
    }    

    _mm512_storeu_ps(dst + 0 * dst_h_stride + 0 * CH_DT_BLK(), zmm5);
    _mm512_storeu_ps(dst + 0 * dst_h_stride + 1 * CH_DT_BLK(), zmm7);
    _mm512_storeu_ps(dst + 1 * dst_h_stride + 0 * CH_DT_BLK(), zmm6);
    _mm512_storeu_ps(dst + 1 * dst_h_stride + 1 * CH_DT_BLK(), zmm8);
}

void winograd_b2f5s2_store_dst_fp32_avx512(
    const float* src,
    const float* sum_src,
    ...
    if (fuse_flag & conv_fuse_flag::SUM) {
        for (int64_t oh = 0; oh < oh_len; ++oh) {
            ...
            for (int64_t ow = 0; ow < ow_len; ++ow) {    
                __m512 tmp = _mm512_add_ps(_mm512_loadu_ps(l_src), _mm512_loadu_ps(l_sum_src));
                tmp = _mm512_max_ps(_mm512_min_ps(tmp, vmax), vmin);
                _mm512_storeu_ps(l_dst, tmp);
                l_src += CH_DT_BLK();
                l_sum_src += CH_DT_BLK();
                l_dst += CH_DT_BLK();
            }
        ...
        for (int64_t oh = 0; oh < oh_len; ++oh) {
            ...
            float* l_dst = dst + oh * dst_h_stride;
            for (int64_t ow = 0; ow < ow_len; ++ow) {    
                __m512 tmp = _mm512_loadu_ps(l_src);
                tmp = _mm512_max_ps(_mm512_min_ps(tmp, vmax), vmin);
                _mm512_storeu_ps(l_dst, tmp);
                l_src += CH_DT_BLK();
                l_dst += CH_DT_BLK();
...
ppl::common::RetCode conv2d_n16cx_winograd_b2f5s2_fp32_avx512_executor::execute()
{
                        ...
                        for (int64_t tk = tl2; tk < tl2 + tl2_eff; tk += TILE_KR_BLK()) {
                            ...
                            for (int64_t icb = icl2; icb < icl2 + icl2_eff_padded; icb += CH_DT_BLK()) {
                                for (int64_t t = 0; t < tk_eff; ++t) {
                                    ...
                                    winograd_b2f5s2_preprocess_fp32_avx512(
                                    ...
                                    for (int64_t tk = tl2; tk < tl2 + tl2_eff; tk += TILE_KR_BLK()) {
                                        ...
                                        for (int64_t t = 0; t < tk_eff; ++t) {
                                            ...
                                            if (oh_len == TILE_OUT_H() && ow_len == TILE_OUT_W()) {
                                                winograd_b2f5s2_dst_trans_fp32_avx512(
                                                ...
                                                winograd_b2f5s2_store_dst_fp32_avx512(
                                                    dst_buf, l_sum_src,
                                                    oh_len, ow_len,
                                                    dst_w * CH_DT_BLK(),
                                                    cp.fuse_flag, l_dst);
...
ppl::common::RetCode conv2d_n16cx_winograd_b2f5s2_fp32_avx512_manager::gen_cvt_weights(
    const float *filter,
    const float *bias)
{
    ...
    // stride = 2, CELL(TILE_IN_H, STRIDE_H)
    const float mat_G[TILE_IN_H() / 2 + 1][KERNEL_W()] = {
        {  1.f,  1.f,   0.f,   0.f,  0.f},
            ...
            for (int64_t ocb = 0; ocb < padded_oc; ocb += CH_DT_BLK()) {
                ...
                for (int64_t ic = icl2; ic < icl2 + icl2_eff; ++ic) {
                    ...
                    for (int64_t oc = 0; oc < ocb_eff; ++oc) {
                        // G * filter
                        for (int64_t i = 0;i < TILE_IN_H(); i += STRIDE_H()) {
                            for (int64_t j = 0; j < KERNEL_W(); ++j) {
                                float sum = 0.0f;
                                for (int64_t k = 0; k < KERNEL_H(); k += STRIDE_H()) {
                                    sum += mat_G[i / 2][k] * l_flt[oc * ic_per_gp * KERNEL_H() * KERNEL_W() + k * KERNEL_W() + j];
                                }
                                mat_T[i][j] = sum;
                                if (i != TILE_IN_H() - 1) {
                                    sum = 0.0f;
                                    for(int64_t k = 1; k < KERNEL_H(); k += STRIDE_H()) {
                                        sum += mat_G[i / 2][k] * l_flt[oc * ic_per_gp * KERNEL_H() * KERNEL_W() + k * KERNEL_W() + j];
                                    }
                                    mat_T[i + 1][j] = sum;
                                }
                        ...
                        for (int64_t i = 0; i < TILE_IN_H(); ++i) {
                            for (int64_t j = 0; j < TILE_IN_W(); j += STRIDE_W()) {
                                float sum = 0.0f;
                                for (int64_t k = 0; k < KERNEL_W(); k += STRIDE_W()) {
                                    sum += mat_T[i][k] * mat_G[j / 2][k];
                                }
                                l_cvt_flt[(i * TILE_IN_W() + j) * padded_oc * icl2_eff + oc] = sum;
                                
                                if (j != TILE_IN_W() - 1) {
                                    sum = 0.0f;
                                    for (int64_t k = 1; k < KERNEL_W(); k += STRIDE_W()){
                                        sum += mat_T[i][k] * mat_G[j / 2][k];
                                    }
                                    l_cvt_flt[(i * TILE_IN_W() + j + 1) * padded_oc * icl2_eff + oc] = sum;
                                }
                            }
                        }
                    }
...
bool conv2d_n16cx_winograd_b2f5s2_fp32_avx512_manager::is_supported() 
{
    ...
    if (param_.channels / param_.group <= 7.5f * CH_DT_BLK()) {
        return false;
    }
...
conv2d_fp32_executor *conv2d_n16cx_winograd_b2f5s2_fp32_avx512_manager::gen_executor()
{
    return new conv2d_n16cx_winograd_b2f5s2_fp32_avx512_executor(&param_, cvt_filter_, cvt_bias_);
}
