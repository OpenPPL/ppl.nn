    ...
    eT *dst)
{
    ...
    for (int64_t d0 = 0; d0 < dst_dims[0]; ++d0) {
        ...
        int64_t inner_stride = map_stride[1];
...
#endif
    for (int64_t d0 = 0; d0 < dst_dims[0]; ++d0) {
        for (int32_t d1 = 0; d1 < dst_dims[1]; ++d1) {
            auto l_src = src
                + d0 * map_stride[0]
                + d1 * map_stride[1];
            auto l_dst = dst
            ...
            int64_t inner_stride = map_stride[2];
    ...
    for (int64_t d0 = 0; d0 < dst_dims[0]; ++d0) {
        for (int32_t d1 = 0; d1 < dst_dims[1]; ++d1) {
            for (int64_t d2 = 0; d2 < dst_dims[2]; ++d2) {
                ...
                int64_t inner_stride = map_stride[3];
        ...
        for (int32_t d1 = 0; d1 < dst_dims[1]; ++d1) {
            for (int64_t d2 = 0; d2 < dst_dims[2]; ++d2) {
                for (int64_t d3 = 0; d3 < dst_dims[3]; ++d3) {
                    ...
                    int64_t inner_stride = map_stride[4];
            ...
            for (int64_t d2 = 0; d2 < dst_dims[2]; ++d2) {
                for (int64_t d3 = 0; d3 < dst_dims[3]; ++d3) {
                    for (int64_t d4 = 0; d4 < dst_dims[4]; ++d4) {
                        ...
                        int64_t inner_stride = map_stride[5];
...
void transpose_ndarray_recursive(
    ...
    eT *dst)
{
    const int64_t length = dst_dims[dim_index];
    if (dim_index == dim_count - 1) {
        if (dim_index == pc.depth_of_loop && length > 1) {
            const int64_t len_per_thread = div_up(length, pc.num_threads);
            PRAGMA_OMP_PARALLEL_FOR()
            for (int64_t t = 0; t < pc.num_threads; ++t) {
                const int64_t beg_idx = t * len_per_thread;
                const int64_t end_idx = min<int64_t>(beg_idx + len_per_thread, length);
                for (int64_t i = beg_idx; i < end_idx; i++) {
                    dst[dst_offset + i] = src[src_offset + i * map_stride[dim_index]];
                }
            }
        } else {
