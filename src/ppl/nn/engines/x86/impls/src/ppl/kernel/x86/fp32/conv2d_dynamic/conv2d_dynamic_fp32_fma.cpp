...
static void batch_im2col_ndarray_fp32_avx(
    ...
    const int32_t hole_w)
{
    ...
    __m256 mmzero        = _mm256_setzero_ps();
                ...
                for (int64_t kw = 0; kw < kernel_w; ++kw) {
                    ...
                    for (int64_t oh = 0; oh < oh_beg; ++oh) {
                        ...
                        for (; ow <= dst_w - simd_w; ow += simd_w) {
                            _mm256_storeu_ps(out_d + oh * dst_w + ow, mmzero);
                        }
                        for (; ow < dst_w; ++ow) {
        ...
        if (bias != nullptr) {
            ...
            for (int64_t ibg = 0; ibg < bg_len_eff; ++ibg) {
                for (int64_t m = 0; m < M; ++m) {
                    __m256 mmbias = _mm256_set1_ps(bias_list[bg + ibg][m]);
                    for (int64_t n = 0; n < round(N, simd_w); n += simd_w) {
                        _mm256_storeu_ps(dst_list[bg + ibg] + m * N + n,
                                         _mm256_add_ps(mmbias, _mm256_loadu_ps(dst_list[bg + ibg] + m * N + n)));
                    }
...
ppl::common::RetCode x86_conv2d_dynamic_ndarray_with_relu_fp32_fma(
    const float *input,
    const float *filter,
    const float *bias,
    const int32_t src_h,
        ...
        if (bias != nullptr) {
            ...
            for (int64_t ibg = 0; ibg < bg_len_eff; ++ibg) {
                for (int64_t m = 0; m < M; ++m) {
                    ...
                    for (int64_t n = 0; n < round(N, simd_w); n += simd_w) {
                        _mm256_storeu_ps(dst_list[bg + ibg] + m * N + n,
                                         _mm256_max_ps(mmzero, _mm256_add_ps(mmbias, _mm256_loadu_ps(dst_list[bg + ibg] + m * N + n))));
                    }
                    for (int64_t n = round(N, simd_w); n < N; ++n) {
            ...
            for (int64_t ibg = 0; ibg < bg_len_eff; ++ibg) {
                for (int64_t m = 0; m < M; ++m) {
                    for (int64_t n = 0; n < round(N, simd_w); n += simd_w) {
                        _mm256_storeu_ps(dst_list[bg + ibg] + m * N + n,
                                         _mm256_max_ps(mmzero, _mm256_loadu_ps(dst_list[bg + ibg] + m * N + n)));
                    }
                    for (int64_t n = round(N, simd_w); n < N; ++n) {
