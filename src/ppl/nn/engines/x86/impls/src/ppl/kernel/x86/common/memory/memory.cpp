...
ppl::common::RetCode memory_copy(
    ...
    void* dst)
{
    const uint64_t min_block_size = 128 * 1024;
    const uint64_t num_block = min<uint64_t>(PPL_OMP_MAX_THREADS(), div_up(num_bytes, min_block_size));
    const uint64_t block_body = num_bytes / num_block;
    const uint64_t block_tail = num_bytes % num_block;
    ...
    for (uint64_t n = 0; n < num_block; ++n) {
        ...
        memcpy(l_dst + block_start, src + block_start, block_size);
    }
