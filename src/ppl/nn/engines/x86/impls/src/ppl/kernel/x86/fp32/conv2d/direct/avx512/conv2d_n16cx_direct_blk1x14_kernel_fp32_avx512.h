...
#define IC_COMPUTE_STEP(IC) do {\
    if (oc_len > 0 * CH_DT_BLK()) zmm28 = _mm512_loadu_ps(icb_flt_o16 + (IC) * CH_DT_BLK());\
    if (oc_len > 1 * CH_DT_BLK()) zmm29 = _mm512_loadu_ps(icb_flt_o32 + (IC) * CH_DT_BLK());\
    if (w_len > 0) {\
    ...
    if (w_len > 11) {\
        zmm31 = _mm512_set1_ps(k_src[(IC) + 11 * src_sw_stride]);\
    ...
    if (w_len > 12) {\
        zmm30 = _mm512_set1_ps(k_src[(IC) + 12 * src_sw_stride]);\
    ...
    if (w_len > 13) {\
        zmm31 = _mm512_set1_ps(k_src[(IC) + 13 * src_sw_stride]);\
...
#define IC_PREFETCH_STEP(IC)  do {\
    if (oc_len > 1 * CH_DT_BLK() && w_len > 6) {\
        ...
        if (oc_len > 1 * CH_DT_BLK()) _mm_prefetch((const char*)icb_flt_o32 + 0 * flt_ocb_stride + (IC) * CH_DT_BLK() + CH_DT_BLK() * CH_DT_BLK(), _MM_HINT_T0);\
    }\
} while (0)
            ...
            for (int64_t kh = kh_start; kh < kh_end; ++kh) {
                for (int64_t kw = 0; kw < kernel_w; ++kw) {
                    for (int64_t ic = 0; ic < channels; ++ic) {
                        ...
                        icb_flt_o32 += CH_DT_BLK();
                    }
                    icb_flt_o16 += (CH_DT_BLK() - channels) * CH_DT_BLK();
                    icb_flt_o32 += (CH_DT_BLK() - channels) * CH_DT_BLK();
