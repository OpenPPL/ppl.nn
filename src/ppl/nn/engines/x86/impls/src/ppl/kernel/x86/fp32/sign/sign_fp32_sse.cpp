    ...
    float *y)
{
    ...
    for (int64_t i = 0; i < unroll_body; i += unroll_n) {
        auto src0 = _mm_loadu_ps(x + i + 0);
        auto src1 = _mm_loadu_ps(x + i + 4);
        auto src2 = _mm_loadu_ps(x + i + 8);
        auto src3 = _mm_loadu_ps(x + i + 12);
        _mm_storeu_ps(y + i + 0, _sse_sign_ps(src0));
        _mm_storeu_ps(y + i + 4, _sse_sign_ps(src1));
        _mm_storeu_ps(y + i + 8, _sse_sign_ps(src2));
        _mm_storeu_ps(y + i + 12, _sse_sign_ps(src3));
    }
