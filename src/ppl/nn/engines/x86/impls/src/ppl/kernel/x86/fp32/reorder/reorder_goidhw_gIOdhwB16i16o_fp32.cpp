    ...
    for (int64_t g = 0; g < group; ++g) {
        for (int64_t ic_big = 0; ic_big < padded_ic; ic_big += ic_big_blk) {
            for (int64_t ocb = 0; ocb < padded_oc; ocb += CH_DT_BLK()) {
                ...
                float *base_dst          = dst + g * ic_big_cnt * padded_oc * kernel_d * kernel_h * kernel_w * ic_big_blk + ic_big * padded_oc * kernel_d * kernel_h * kernel_w + ocb * kernel_d * kernel_h * kernel_w * ic_big_blk;
                    ...
                    for (int64_t kh = 0; kh < kernel_h; ++kh) {
                        for (int64_t kw = 0; kw < kernel_w; ++kw) {
                            for (int64_t icb = 0; icb < ic_big_eff; icb += CH_DT_BLK()) {
                                ...
                                float *l_dst          = base_dst + kd * kernel_h * kernel_w * ic_big_blk * CH_DT_BLK() + kh * kernel_w * ic_big_blk * CH_DT_BLK() + kw * ic_big_blk * CH_DT_BLK() + icb * CH_DT_BLK();
                                int64_t ic            = 0;
                                for (; ic < icb_eff; ++ic) {
                                    int64_t oc = 0;
                                    for (; oc < ocb_eff; ++oc) {
