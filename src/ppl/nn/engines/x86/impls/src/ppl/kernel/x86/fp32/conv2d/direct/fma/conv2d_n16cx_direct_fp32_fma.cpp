...
bool conv2d_n16cx_direct_fp32_fma_executor::init_profiler()
{
...
void conv2d_n16cx_direct_fp32_fma_executor::clear_profiler()
{
#ifdef PPL_X86_KERNEL_TIMING
    profiler_.clear();
#endif
...
std::string conv2d_n16cx_direct_fp32_fma_executor::export_profiler()
{
#ifdef PPL_X86_KERNEL_TIMING
...
int32_t conv2d_n16cx_direct_fp32_fma_executor::cal_ic_l2_blk(const conv2d_fp32_param &param)
{
...
void conv2d_n16cx_direct_fp32_fma_executor::cal_kernel_threading_param(const int32_t batch, const int32_t group, const int32_t ic)
{
    const conv2d_fp32_param &cp = *conv_param_;
    kernel_schedule_param &sp   = schedule_param_;
                                ...
                                for (int64_t ocl2 = 0; ocl2 < padded_rf_oc; ocl2 += sp.oc_l2_blk) {
                                    for (int64_t ohl2 = 0; ohl2 < ohl3_eff; ohl2 += sp.oh_l2_blk) {
                                        for (int64_t owl2 = 0; owl2 < owl3_eff; owl2 += sp.ow_l2_blk) {
                                            ...
                                            const int64_t stride_sel        = cp.stride_w - 1;
                                            ...
                                            for (int64_t oc = ocl2; oc < ocl2 + ocl2_eff; oc += CH_DT_BLK()) {
                                                ...
                                                if (ow_body) {
                                                    ...
                                                    *reinterpret_cast<float **>(kernel_param + DST_IDX())       = l_dst;
                                                    ...
                                                    conv2d_n16cx_direct_kernel_fp32_fma_blk1x6_table[stride_sel][nt_store_sel][prefetch_sel][oc_sel][sp.ow_kr_blk - 1](kernel_param, share_param);
                                                }
                                                if (ow_tail) {
                                                    ...
                                                    *reinterpret_cast<float **>(kernel_param + DST_IDX())       = l_dst + ow_body * CH_DT_BLK();
                                                    ...
                                                    conv2d_n16cx_direct_kernel_fp32_fma_blk1x6_table[stride_sel][nt_store_sel][prefetch_sel][oc_sel][ow_tail - 1](kernel_param, share_param);
                                                }
...
ppl::common::RetCode conv2d_n16cx_direct_fp32_fma_manager::gen_cvt_weights(const float *filter, const float *bias)
{
    ...
    const int32_t ic_l2_blk = conv2d_n16cx_direct_fp32_fma_executor::cal_ic_l2_blk(param_);
...
bool conv2d_n16cx_direct_fp32_fma_manager::is_supported()
{
    ...
    if (param_.stride_w > 2) {
        return false;
    }
    if (param_.sparse_level() > 3.99f) {
        return false;
    }
