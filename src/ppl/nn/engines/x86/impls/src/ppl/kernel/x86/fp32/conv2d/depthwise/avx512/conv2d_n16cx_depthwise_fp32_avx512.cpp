...
void conv2d_n16cx_depthwise_fp32_avx512_executor::init_preproc_param()
{
    schedule_param_.padded_ch = round_up(conv_param_->group, CH_DT_BLK());
    schedule_param_.ow_kr_blk = MAX_OW_RF();
}

void conv2d_n16cx_depthwise_fp32_avx512_executor::cal_kernel_tunning_param()
{
    ...
    const int64_t src_len     = int64_t(batch) * sp.padded_ch * src_h * src_w;
    const int64_t dst_len     = int64_t(batch) * sp.padded_ch * dst_h * dst_w;
...
uint64_t conv2d_n16cx_depthwise_fp32_avx512_executor::cal_temp_buffer_size()
{
...
ppl::common::RetCode conv2d_n16cx_depthwise_fp32_avx512_executor::prepare()
{
...
ppl::common::RetCode conv2d_n16cx_depthwise_fp32_avx512_executor::execute()
{
    ...
    const int64_t padded_src_h_stride = int64_t(src_w + 2 * cp.pad_w) * CH_DT_BLK();
...
ppl::common::RetCode conv2d_n16cx_depthwise_fp32_avx512_manager::gen_cvt_weights(const float *filter, const float *bias)
{
...
bool conv2d_n16cx_depthwise_fp32_avx512_manager::is_supported()
{
    return param_.is_depthwise();
}

conv2d_fp32_executor *conv2d_n16cx_depthwise_fp32_avx512_manager::gen_executor()
{
    return new conv2d_n16cx_depthwise_fp32_avx512_executor(&param_, cvt_filter_, cvt_bias_);
}
