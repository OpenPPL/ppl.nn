    ...
    const int32_t mode)
{
    ...
    return round_up(min<int64_t>(TILE_L2_BLK_MAX_L(), num_tiles), TILE_KR_BLK());
}
...
static const winograd_b6f3_kernel_fp32_sse_func_t winograd_b6f3_transpose_4x4_fp32_sse_func_table[2][CH_RF_BLK() + 1]{
    {
        wingorad_b6f3_transpose_4x4_fp32_sse<false, 0, CH_DT_BLK()>,
    ...
    {
        wingorad_b6f3_transpose_4x4_fp32_sse<true, 0, CH_DT_BLK()>,
...
template <uint64_t fuse_flag, int64_t channel>
    ...
    float *dst)
{
    ...
    winograd_b6f3_dst_trans_fp32_sse<fuse_flag, channel>(matmul_buffer + 0 * matmul_h_stride, sum_src + 0 * dst_h_stride, dst_hw, tmp_hw, dst + 0 * tmp_h_stride);
    winograd_b6f3_dst_trans_fp32_sse<fuse_flag, channel>(matmul_buffer + 1 * matmul_h_stride, sum_src + 1 * dst_h_stride, dst_hw, tmp_hw, dst + 1 * tmp_h_stride);
    winograd_b6f3_dst_trans_fp32_sse<fuse_flag, channel>(matmul_buffer + 2 * matmul_h_stride, sum_src + 2 * dst_h_stride, dst_hw, tmp_hw, dst + 2 * tmp_h_stride);
    winograd_b6f3_dst_trans_fp32_sse<fuse_flag, channel>(matmul_buffer + 3 * matmul_h_stride, sum_src + 3 * dst_h_stride, dst_hw, tmp_hw, dst + 3 * tmp_h_stride);
    winograd_b6f3_dst_trans_fp32_sse<fuse_flag, channel>(matmul_buffer + 4 * matmul_h_stride, sum_src + 4 * dst_h_stride, dst_hw, tmp_hw, dst + 4 * tmp_h_stride);
    winograd_b6f3_dst_trans_fp32_sse<fuse_flag, channel>(matmul_buffer + 5 * matmul_h_stride, sum_src + 5 * dst_h_stride, dst_hw, tmp_hw, dst + 5 * tmp_h_stride);
}
...
static const winograd_b6f3_preprocess_fp32_sse_func_t winograd_b6f3_preprocess_fp32_sse_func_table[CH_DT_BLK() + 1]{
    winograd_b6f3_preprocess_fp32_sse<0>,
...
typedef void (*winograd_b6f3_postprocess_fp32_sse_func_t)(const float *, const float *, const float *, const int64_t, const int64_t, const int64_t, const bool, float *, float *, float *);
static const winograd_b6f3_postprocess_fp32_sse_func_t winograd_b6f3_postprocess_fp32_sse_func_table[FLAG_CNT()][CH_DT_BLK() + 1]{
    {
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 0>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 1>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 2>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 3>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 4>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 5>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 6>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 7>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::NONE, 8>,
    },
    {
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 0>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 1>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 2>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 3>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 4>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 5>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 6>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 7>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU, 8>,
    },
    {
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 0>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 1>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 2>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 3>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 4>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 5>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 6>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 7>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::RELU6, 8>,
    },
    {
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 0>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 1>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 2>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 3>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 4>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 5>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 6>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 7>,
        winograd_b6f3_postprocess_fp32_sse<conv_fuse_flag::SUM, 8>,
    },
...
static const winograd_b6f3_store_dst_fp32_sse_func_t winograd_b6f3_store_dst_fp32_sse_func_table[CH_DT_BLK() + 1]{
    winograd_b6f3_store_dst_fp32_sse<0>,
...
ppl::common::RetCode conv2d_winograd_b6f3_fp32_sse_executor::execute()
{
    ...
    const uint64_t fuse_flag       = min<uint64_t>(cp.fuse_flag, FLAG_CNT() - 1);
        ...
        for (int64_t tl2 = 0; tl2 < sp.num_tiles; tl2 += sp.tiles_l2_blk) {
            ...
            for (int64_t icl2 = 0; icl2 < sp.ic_per_gp; icl2 += sp.ic_l2_blk) {
                ...
                if (is_first_ic) {
                    ...
                    float tmp_bias[TILE_OC_BLK() * CH_DT_BLK()];
                    memset32_sse(tmp_bias, 0, TILE_OC_BLK() * CH_DT_BLK());
                    l_bias = tmp_bias;
                }
                ...
                for (int64_t ocl2 = 0; ocl2 < sp.padded_oc; ocl2 += sp.oc_l2_blk) {
                    ...
                    for (int64_t ti = 0; ti < TILE_IN_H() * TILE_IN_W(); ++ti) {
                        for (int64_t ocb = ocl2; ocb < ocl2 + ocl2_eff; ocb += CH_DT_BLK() * TILE_OC_BLK()) {
#ifdef PPL_X86_KERNEL_TIMING
                            ...
                            const int64_t oc_sel              = min<int64_t>(TILE_OC_BLK(), div_up(ocl2_eff - ocb + ocl2, CH_DT_BLK())) - 1;
                            ...
                            if (t_body) {
                                ...
                                private_param[HW_IDX()]                             = t_body;
                                conv2d_n8cx_gemm_direct_kernel_fp32_sse_hw1_table[0][oc_sel](private_param, share_param);
                            }
                            if (t_tail) {
                                PICK_PARAM(const float *, private_param, SRC_IDX()) = l_src_trans + t_body * CH_DT_BLK();
                                PICK_PARAM(const float *, private_param, HIS_IDX()) = l_gemm_out + t_body * CH_DT_BLK();
                                PICK_PARAM(float *, private_param, DST_IDX())       = l_gemm_out + t_body * CH_DT_BLK();
                                private_param[HW_IDX()]                             = t_tail;
                                conv2d_n8cx_gemm_direct_kernel_fp32_sse_hw1_table[0][oc_sel](private_param, share_param);
                            }
                        ...
                        for (int64_t ocb = ocl2; ocb < ocl2 + ocl2_eff; ocb += CH_DT_BLK()) {
                            for (int64_t tk = tl2; tk < tl2 + tl2_eff; ++tk) {
                                ...
                                if (oh_len == TILE_OUT_H() && ow_len == TILE_OUT_W()) {
                                    winograd_b6f3_postprocess_fp32_sse_func_table[fuse_flag][channel](
                                        l_gemm_out, l_sum_src, cvt_bias_ + g * bias_g_stride + ocb, 
                                        gemm_out_ti_stride, dst_w, dst_hw, true, postprocess_buf, dst_buf, l_dst);

                                } else {
                                    winograd_b6f3_postprocess_fp32_sse_func_table[fuse_flag][channel](
                                        l_gemm_out, l_sum_src, cvt_bias_ + g * bias_g_stride + ocb, 
                                        gemm_out_ti_stride, dst_w, dst_hw, false, postprocess_buf, dst_buf, dst_buf);
