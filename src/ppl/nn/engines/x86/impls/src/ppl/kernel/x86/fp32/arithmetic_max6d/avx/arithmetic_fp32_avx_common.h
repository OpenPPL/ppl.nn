#ifndef __ST_PPL_KERNEL_X86_FP32_ARITHMETIC_MAX6D_AVX_ARITHMETIC_FP32_AVX_COMMON_H_
#define __ST_PPL_KERNEL_X86_FP32_ARITHMETIC_MAX6D_AVX_ARITHMETIC_FP32_AVX_COMMON_H_
...
void arithmetic_binary_op_ndarray_6d_broadcast_fp32_avx(
                ...
                for (int64_t d3 = 0; d3 < dst_dims[3]; ++d3) {
                    for (int64_t d4 = 0; d4 < dst_dims[4]; ++d4) {
                        ...
                        for (int64_t ib = 0; ib < dst_dims[5]; ib += inner_blk) {
                            ...
                            if (unroll_body) {
                                __m256 mm_broadcast = _mm256_set1_ps(broadcast_src[0]);
                                for (int64_t i = 0; i < unroll_body; i += unroll_len) {
                                    __m256 mm_src0 = _mm256_loadu_ps(plain_src + i + 0 * simd_w);
                                    __m256 mm_src1 = _mm256_loadu_ps(plain_src + i + 1 * simd_w);
                                    __m256 mm_src2 = _mm256_loadu_ps(plain_src + i + 2 * simd_w);
                                    __m256 mm_src3 = _mm256_loadu_ps(plain_src + i + 3 * simd_w);
                                    if (_op == ARITHMETIC_ADD) {
                                        mm_src0 = _mm256_add_ps(mm_src0, mm_broadcast);
                                        mm_src1 = _mm256_add_ps(mm_src1, mm_broadcast);
                                        mm_src2 = _mm256_add_ps(mm_src2, mm_broadcast);
                                        mm_src3 = _mm256_add_ps(mm_src3, mm_broadcast);
                                    } else if (_op == ARITHMETIC_MUL) {
                                        mm_src0 = _mm256_mul_ps(mm_src0, mm_broadcast);
                                        mm_src1 = _mm256_mul_ps(mm_src1, mm_broadcast);
                                        mm_src2 = _mm256_mul_ps(mm_src2, mm_broadcast);
                                        mm_src3 = _mm256_mul_ps(mm_src3, mm_broadcast);
                                    }
                                    if (broadcast_side == 0) {
                                        if (_op == ARITHMETIC_DIV) {
                                            mm_src0 = _mm256_div_ps(mm_broadcast, mm_src0);
                                            mm_src1 = _mm256_div_ps(mm_broadcast, mm_src1);
                                            mm_src2 = _mm256_div_ps(mm_broadcast, mm_src2);
                                            mm_src3 = _mm256_div_ps(mm_broadcast, mm_src3);
                                        } else if (_op == ARITHMETIC_SUB) {
                                            mm_src0 = _mm256_sub_ps(mm_broadcast, mm_src0);
                                            mm_src1 = _mm256_sub_ps(mm_broadcast, mm_src1);
                                            mm_src2 = _mm256_sub_ps(mm_broadcast, mm_src2);
                                            mm_src3 = _mm256_sub_ps(mm_broadcast, mm_src3);
                                        }
                                    } else if (broadcast_side == 1) {
                                        if (_op == ARITHMETIC_DIV) {
                                            mm_src0 = _mm256_div_ps(mm_src0, mm_broadcast);
                                            mm_src1 = _mm256_div_ps(mm_src1, mm_broadcast);
                                            mm_src2 = _mm256_div_ps(mm_src2, mm_broadcast);
                                            mm_src3 = _mm256_div_ps(mm_src3, mm_broadcast);
                                        } else if (_op == ARITHMETIC_SUB) {
                                            mm_src0 = _mm256_sub_ps(mm_src0, mm_broadcast);
                                            mm_src1 = _mm256_sub_ps(mm_src1, mm_broadcast);
                                            mm_src2 = _mm256_sub_ps(mm_src2, mm_broadcast);
                                            mm_src3 = _mm256_sub_ps(mm_src3, mm_broadcast);
                                        }
                                    }
                                    _mm256_storeu_ps(l_dst + i + 0 * simd_w, mm_src0);
                                    _mm256_storeu_ps(l_dst + i + 1 * simd_w, mm_src1);
                                    _mm256_storeu_ps(l_dst + i + 2 * simd_w, mm_src2);
                                    _mm256_storeu_ps(l_dst + i + 3 * simd_w, mm_src3);
                                }
                            }
...
void arithmetic_binary_op_ndarray_6d_eltwise_fp32_avx(
    const int64_t lhs_strides[5],
    const int64_t rhs_strides[5],
    const int64_t dst_strides[5],
    const int64_t dst_dims[6],
                ...
                for (int64_t d3 = 0; d3 < dst_dims[3]; ++d3) {
                    for (int64_t d4 = 0; d4 < dst_dims[4]; ++d4) {
                        ...
                        for (int64_t ib = 0; ib < dst_dims[5]; ib += inner_blk) {
                            ...
                            for (int64_t i = 0; i < unroll_body; i += unroll_len) {
                                __m256 mm_src0 = _mm256_loadu_ps(l_rhs + i + 0 * simd_w);
                                __m256 mm_src1 = _mm256_loadu_ps(l_rhs + i + 1 * simd_w);
                                __m256 mm_src2 = _mm256_loadu_ps(l_rhs + i + 2 * simd_w);
                                __m256 mm_src3 = _mm256_loadu_ps(l_rhs + i + 3 * simd_w);
                                if (_op == ARITHMETIC_ADD) {
                                    mm_src0 = _mm256_add_ps(_mm256_loadu_ps(l_lhs + i + 0 * simd_w), mm_src0);
                                    mm_src1 = _mm256_add_ps(_mm256_loadu_ps(l_lhs + i + 1 * simd_w), mm_src1);
                                    mm_src2 = _mm256_add_ps(_mm256_loadu_ps(l_lhs + i + 2 * simd_w), mm_src2);
                                    mm_src3 = _mm256_add_ps(_mm256_loadu_ps(l_lhs + i + 3 * simd_w), mm_src3);
                                } else if (_op == ARITHMETIC_SUB) {
                                    mm_src0 = _mm256_sub_ps(_mm256_loadu_ps(l_lhs + i + 0 * simd_w), mm_src0);
                                    mm_src1 = _mm256_sub_ps(_mm256_loadu_ps(l_lhs + i + 1 * simd_w), mm_src1);
                                    mm_src2 = _mm256_sub_ps(_mm256_loadu_ps(l_lhs + i + 2 * simd_w), mm_src2);
                                    mm_src3 = _mm256_sub_ps(_mm256_loadu_ps(l_lhs + i + 3 * simd_w), mm_src3);
                                } else if (_op == ARITHMETIC_MUL) {
                                    mm_src0 = _mm256_mul_ps(_mm256_loadu_ps(l_lhs + i + 0 * simd_w), mm_src0);
                                    mm_src1 = _mm256_mul_ps(_mm256_loadu_ps(l_lhs + i + 1 * simd_w), mm_src1);
                                    mm_src2 = _mm256_mul_ps(_mm256_loadu_ps(l_lhs + i + 2 * simd_w), mm_src2);
                                    mm_src3 = _mm256_mul_ps(_mm256_loadu_ps(l_lhs + i + 3 * simd_w), mm_src3);
                                } else if (_op == ARITHMETIC_DIV) {
                                    mm_src0 = _mm256_div_ps(_mm256_loadu_ps(l_lhs + i + 0 * simd_w), mm_src0);
                                    mm_src1 = _mm256_div_ps(_mm256_loadu_ps(l_lhs + i + 1 * simd_w), mm_src1);
                                    mm_src2 = _mm256_div_ps(_mm256_loadu_ps(l_lhs + i + 2 * simd_w), mm_src2);
                                    mm_src3 = _mm256_div_ps(_mm256_loadu_ps(l_lhs + i + 3 * simd_w), mm_src3);
                                }
                                _mm256_storeu_ps(l_dst + i + 0 * simd_w, mm_src0);
                                _mm256_storeu_ps(l_dst + i + 1 * simd_w, mm_src1);
                                _mm256_storeu_ps(l_dst + i + 2 * simd_w, mm_src2);
...
template <arithmetic_op_type_t _op>
ppl::common::RetCode arithmetic_binary_op_ndarray_fp32_avx(
    ...
    float *dst)
{
    ...
    if (eltwise) {
        for (uint32_t i = 0; i < lhs_shape->GetDimCount(); ++i) {
    ...
    if (simd_broadcast) {
        if (broadcast_side == 0) {
            arithmetic_binary_op_ndarray_6d_broadcast_fp32_avx<_op, 0>(
                lhs_strides, rhs_strides, dst_strides, dst_dims, lhs, rhs, dst);
        } else {
            arithmetic_binary_op_ndarray_6d_broadcast_fp32_avx<_op, 1>(
                lhs_strides, rhs_strides, dst_strides, dst_dims, lhs, rhs, dst);
        }
    } else {
        arithmetic_binary_op_ndarray_6d_eltwise_fp32_avx<_op>(lhs_strides, rhs_strides, dst_strides, dst_dims, lhs, rhs, dst);
    }
...
#endif // __ST_PPL_KERNEL_X86_FP32_ARITHMETIC_MAX6D_AVX_ARITHMETIC_FP32_AVX_COMMON_H_
