            ...
            for (int64_t b = 0; b < batch; ++b) {
                ...
                if (seq_idx < seq_end) {
                    ...
                    for (; h <= hidden_size - simd_w; h += simd_w) {
                        auto zt = _fma_sigmoid_ps(_mm256_loadu_ps(gZ + h));
                        auto rt = _fma_sigmoid_ps(_mm256_loadu_ps(gR + h));
                        auto ht = _fma_tanh_ps(rt * _mm256_loadu_ps(gE + h) + _mm256_loadu_ps(gH + h));
                        _mm256_storeu_ps(Ht + h, ht - zt * ht + zt * _mm256_loadu_ps(Hprev + h));
                    }
                    for (; h < hidden_size; ++h) {
