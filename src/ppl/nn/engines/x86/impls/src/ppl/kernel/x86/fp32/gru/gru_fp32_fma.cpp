    ...
    float *Y_h)
{
    ...
    for (int64_t nd = 0; nd < num_direction; ++nd) {
        ...
        for (int64_t seq_idx = 0; seq_idx < seq_len; ++seq_idx) {
            ...
            gemm_fp32_fma( // Xt*(Wz^T) + Wbz ; Xt*(Wr^T) + Wbr ; Xt*(Wh^T) + Wbh
                sX,
                nd_W,
                nd_Wb,
                nullptr,
            ...
            gemm_fp32_fma( // Ht-1*(Rz^T) + Rbz; Ht-1*(Rr^T) + Rbr
                Y_h_prev,
                nd_Rzr,
                nd_Rb,
                nullptr,
            ...
            for (int64_t b = 0; b < batch; ++b) {
                if (!sequence_lens || seq_idx < sequence_lens[b]) {
                    ...
                    for (; h <= hidden_size - simd_w; h += simd_w) {
                        const __m256 zt   = _fma_sigmoid_ps(_mm256_loadu_ps(gZ + h));
                        const __m256 rt   = _fma_sigmoid_ps(_mm256_loadu_ps(gR + h));
                        const __m256 et   = _mm256_loadu_ps(gE + h);
                        __m256 ht         = _fma_tanh_ps(rt * et + _mm256_loadu_ps(gH + h));
                        const __m256 hpre = _mm256_loadu_ps(Hprev + h);
                        const __m256 res  = (vone - zt) * ht + zt * hpre;
                        _mm256_storeu_ps(Ht + h, res);
                    }
                    for (; h < hidden_size; ++h) {
