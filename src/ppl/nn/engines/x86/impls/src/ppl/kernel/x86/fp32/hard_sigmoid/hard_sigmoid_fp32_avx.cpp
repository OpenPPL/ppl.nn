    ...
    float *dst)
{
    ...
    if (unroll_body) {
        ...
        for (int64_t i = 0; i < unroll_body; i += unroll_len) {
            ...
            v_src0 = _mm256_mul_ps(v_src0, v_alpha);
            v_src1 = _mm256_mul_ps(v_src1, v_alpha);

            v_src0 = _mm256_add_ps(v_src0, v_beta);
            v_src1 = _mm256_add_ps(v_src1, v_beta);

            v_src0 = _mm256_min_ps(v_src0, v_one);
            v_src1 = _mm256_min_ps(v_src1, v_one);

            v_src0 = _mm256_max_ps(v_src0, v_zero);
            v_src1 = _mm256_max_ps(v_src1, v_zero);

            _mm256_storeu_ps(dst + i + 0 * V_REG_ELTS, v_src0);
            _mm256_storeu_ps(dst + i + 1 * V_REG_ELTS, v_src1);
        }
    }
