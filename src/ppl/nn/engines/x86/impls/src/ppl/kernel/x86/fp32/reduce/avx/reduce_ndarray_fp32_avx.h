...
#ifndef __ST_PPL_KERNEL_X86_FP32_REDUCE_AVX_REDUCE_NDARRAY_FP32_AVX_H_
#define __ST_PPL_KERNEL_X86_FP32_REDUCE_AVX_REDUCE_NDARRAY_FP32_AVX_H_
...
static void reduce_ndarray_lastdim_no_reduce_fp32_avx(
    const float *src,
    const int64_t width,
    float *dst)
{
    ...
    for (; i + unroll_len <= width; i += unroll_len) {
        __m256 v_src_0 = _mm256_loadu_ps(src + i + simd_w * 0);
        __m256 v_src_1 = _mm256_loadu_ps(src + i + simd_w * 1);
        __m256 v_src_2 = _mm256_loadu_ps(src + i + simd_w * 2);
        __m256 v_src_3 = _mm256_loadu_ps(src + i + simd_w * 3);
...
template <reduce_op_type_t _op>
static void reduce_ndarray_lastdim_reduce_fp32_avx(
    const float *src,
    const int64_t width,
    float *dst)
{
    const float init_val    = reduce_init_val_fp32<_op>();
    const __m256 v_srcit_val = _mm256_set1_ps(init_val);

    const int64_t simd_w     = 8;
    const int64_t unroll_len = simd_w * 4;
    float reduce_val          = init_val;
    __m256 v_reduce_val_0     = v_srcit_val;
    __m256 v_reduce_val_1     = v_srcit_val;
    __m256 v_reduce_val_2     = v_srcit_val;
    __m256 v_reduce_val_3     = v_srcit_val;
    ...
    if (width >= unroll_len) {
        v_reduce_val_0 = reduce_vector_kernel_fp32_avx<_op>(v_reduce_val_0, v_reduce_val_1);
        v_reduce_val_2 = reduce_vector_kernel_fp32_avx<_op>(v_reduce_val_2, v_reduce_val_3);
        v_reduce_val_0 = reduce_vector_kernel_fp32_avx<_op>(v_reduce_val_0, v_reduce_val_2);
        reduce_val     = reduce_scalar_kernel_fp32<_op>(reduce_vector_all_lanes_kernel_fp32_avx<_op>(v_reduce_val_0), reduce_val);
    }
    ...
    float *dst)
{
    ...
    if (dim_idx == src_shape->GetDimCount() - 1) { // last dim
        if (src_shape->GetDim(dim_idx) == dst_shape->GetDim(dim_idx)) {
            reduce_ndarray_lastdim_no_reduce_fp32_avx<_op>(src, src_shape->GetDim(dim_idx), dst);
        } else { // reduce on last dim_idx
            reduce_ndarray_lastdim_reduce_fp32_avx<_op>(src, src_shape->GetDim(dim_idx), dst);
        }
    } else {
        if (pc->depth_of_loop == dim_idx && pc->num_threads > 1) {  // parallel on this dim
            ...
            for (int64_t t = 0; t < pc->num_threads; t++) {
                ...
                for (int64_t i = start_idx; i < end_idx; i++) {
                    ...
                    reduce_ndarray_recursive_fp32_avx<_op>(src_shape, dst_shape, p_src, dim_idx + 1, inc_src, inc_dst, pc, p_dst);
                }
            }
        } else {
    ...
    float *dst)
{
    ...
    // reduce
    reduce_ndarray_recursive_fp32_avx<_op>(src_shape, &padded_dst_shape, src, 0, inc_src, inc_dst, &pc, dst);
...
#endif // !__ST_PPL_KERNEL_X86_FP32_REDUCE_AVX_REDUCE_NDARRAY_FP32_AVX_H_
